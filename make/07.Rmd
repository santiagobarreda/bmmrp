...
In the last two chapters we talked about comparing two groups. Although the comparison of two groups is very simple, it also comes up often. In addition, more complicated problems are often broken down into several two-group questions in the course of understanding a data set. However, most real-world research designs don't usually *begin* as two-group questions. In this chapter, we're going to talk about the comparison of multiple groups of observations/speakers. In terms of model design concepts, this chapter we're going to discuss how to build models with multiple fixed effect predictions how to allow these factors to *interact* in our models. In the last chapters we've been making the *details* of our models more and more complicated, but we have still only fit models with extremely simple fixed effects structure. In this chapter we're going to introduce more complexity into our fixed effects model structure instead. 

## Comparing four (or any number of) groups

First, we're going to discuss models that compare observations across any number of groups (levels) for a single factor. These designs have just one predictor factor but it can have any number of levels. Although it's not very common to have a design that *only* consists of a single multi-group comparison, the inclusion of multi-group comparisons is ubiquitous in many disciplines (including linguistics). Examples of predictors with multiple levels are vowel phonemes, language, word classes, and ages. In addition, the manipulations researchers carry out in their experiments can vary across more than two groups. For example, consider the hypothetical example of the reading experiment where subjects are asked to drink either a cup of water or a cup of coffee. After a 30-minute wait they are asked to read a passage aloud and the duration of the reading is measured. Let's imagine that subjects were also divided into groups of male and female speakers, and that both men and women were included in both the coffee and water conditions.  This design results in four different groups: Women with water, women with coffee, men with water and men with coffee. These groups have an internal structure that our model does not recognize. What I mean by this is that, for example, half our groups are caffeine groups. However, although you know this your model does not since it does not include exposure to coffee as a predictor. However, we will ignore this issue for now.

In figure \@ref(fig:F71) we see two versions that subjects (or listeners, listeners or whatever) can be arranged into groups. In the *within-subjects* version of this design, each subject is tested at each level of $A$. In the *between-subjects* version of the design, different subjects are tested at every level of $A$. We omit the 'bad' design discussed in chapter 5, where some subjects appear at all levels of $A$ and others don't. Such 'bad' designs are definitely analyzable with `brms`, however, they are not included in figure \@ref(fig:F71) because there are a large number of possible configurations of this, and because it is usually a bad idea to purposely create experiments featuring such designs. 
...
We're going to use the complete data (all speakers) to investigate the effect of apparent speaker category on apparent speaker height. In figure \@ref(fig:71) we see that the distribution of apparent height with respect to apparent speaker category is very similar to its distribution with respect to veridical speaker category. In this chapter we're going to focus on the effect of *apparent* speaker category on apparent height. This is because we want to understand how the listener's assumptions about the speaker affect their height judgments (and listeners did not know the veridical speaker classes). We're first going to treat the four groups as if they had no internal structure. In this case it may not be the best approach for this data, since we know there are logical ways to subdivide men, women, boys and girls. However, this is a good starting point since in many cases you will have levels of a factor with no logical internal divisions. 
...
In terms of research questions, even with only four different groups, our potential research questions are substantially more complicated than in the two-group case. First, there are four groups now, meaning we could potentially make 6 2-group comparisons. Second, the groups differ along multiple dimensions, making it more difficult to make two-group comparisons that ask one single question. For example, the 'man' and 'girl' groups differ according to apparent adultness *and* apparent gender. How could we know what part of their apparent height difference we should attribute to adultness and what part we should attribute to gender? In any case we are going to leave it vague for now and just focus on the fitting and interpretation of models including multi-group fixed effects predictors. 
...
Where `C` is the apparent speaker category indicated by the listener for each trial. Since we include `C` on the left-hand-side of the pipe of `L`, we know that we are estimating the category by listener interaction using adaptive partial pooling. The formal description of our model is given below:

$$
\begin{equation}
\begin{split}
height_{[i]} \sim \mathcal{N}(\mu_{[i]},\sigma) \\ 
\mu_{[i]} = \mathrm{Intercept} + C_{[C_{[i]}]}  + L_{[L_{[i]}]} + C \colon L_{[L_{[i]}]} + S_{[S_{[i]}]} \\ \\ 
\mathrm{Priors:} \\ 
S_{[\bullet]} \sim N(0,\sigma_S) \\
\begin{bmatrix} L_{[\bullet]} \\ C \colon L_{[\bullet]} \end{bmatrix} \sim \mathrm{MVNormal} ( \begin{bmatrix} 0 \\ 0 \\ \end{bmatrix}, \Sigma) \\ \\
\\
...
Structurally, this model is the same as the model described in \@ref(eq:66b) last chapter with $C_{[C_{[i]}]}$ replacing $A_{[A_{[i]}]}$ and $C \colon L_{[L_{[i]}]}$ replacing $A \colon L_{[L_{[i]}]}$. Since we are including adult male speakers in our data we should also reconsider our priors. Based on CDC (cite) we expect that average heights of 176, 162, 150 and 150 cm respectively for men, women, girls, and boys. These four groups have an average of 159 cm and a standard deviation of 12 cm. Based on this, and especially given that we are investigating *apparent* and not *veridical* height, the priors we used in the last chapter still seem appropriate.
...
We can see that the intercept corresponds to the average of the group means, and our `C` coefficients correspond to the centered group means. 
...
We can use the `brmplot` in the `bmmb` package to easily make plots showing means and credible intervals for our fixed effects, including the recovered coefficient. We also plot the two comparisons we made using the `hypothesis` function above. The distributions on the left tell us about probable values for our parameters: Given our data and model structure, there is a 95% probability that the 'real' [@@ SB - :| ... ] parameter value falls within this interval. Our category effects reflect the expected difference between our model intercept, the overall grand mean, and each category mean. So, the intervals on the left of figure \@ref(fig:F72) tell us that there is a 95% probability (given our data and model structure) that the actual difference between the grand mean and each group mean falls within that range. What do the distributions in the right plot in figure \@ref(fig:F72) tell us? They represent the posterior distribution of the difference between category means directly, without involving the intercept. Conceptually, finding the difference between the intercept and a group mean is no different than finding the difference between individual group means. Consider the example of providing directions with respect to your house. If the library is 5 miles west and the park is 3 miles away, you could refer to these as -5 and -3 miles from your house, west to east. However, you could also express the difference between one and the other directly as 2 (or -2). The left plot in figure \@ref(fig:F72) expresses all distances with respect to the intercept (your house) while the right plot expresses differences with respect to individual category means (the library and park). 

```{r F72, fig.height = 3, fig.width=8, fig.cap = "(left) bla. (right) bla.", echo = FALSE}
...
Our model above had a single factor with four groups/levels. Another way to look at it is that the model predicted variation along a single **dimension**, that is some information that can be used to distinguish our groups. However, we can categorize people (or observations) along multiple dimensions simultaneously. So, a speaker may be an adult *and* female, and not just adult *or* female. This is because adultness and gender are independent characteristics and the value of one does not affect the value of the other. In our height experiment we effectively asked listeners to make two binary classifications: Is the listener an adult or a child? And is the listener female or male? This means we think of our groups as varying along two dimensions, each represented by a different factor. However, although we know this our model doesn't, it think we just have four groups that have no logical internal distinctions. 

Our models can include any number of factors, and these can be **crossed** or **nested**. When two factors are crossed, this means that all levels of a factor appear at all levels of another factor. Crossing factors results in $n_A \times n_B$ individual groups for two factors with $n_A$ and $n_B$ number of levels each. Since we have two factors with two levels each, crossing age and gender results in four unique groups: Boys, girls, men, and women. When one or more levels of a factor cannot be observed at all levels of another factor, then these are no longer crossed. When levels of one factor appear exclusively at specific levels of the other factor they are said to be *nested*. For example, consider a situation where men and women where listeners are divided according to native language. The listener factor will necessarily be nested within first language because each speaker only has a single first language. Designs where all the factors are crossed are sometimes called 'factorial' or 'orthogonal' designs. The name 'orthogonal' is particularly telling. It means 'at right angles, perpendicular' (from the greek stems "ortho-", meaning straight, right, and "-gon-", meaning side or angle). When you have orthogonal designs, that means that your 'dimensions' (i.e., factors) can be estimated independently. For example, if you want to know about the difference in apparent height according to apparent gender, it helps to have observed both adults and children for each gender. A comparison of boys and women will conflate gender and age differences, thereby making it difficult to understand the independent effect of apparent gender. So, orthogonal designs allow us to 'decompose' the variation in our data using different factors more effectively. This allows us to attribute the variation in our observations to the different factors in our design, helping us understand what matters and what does not. This simple but extremely powerful idea forms the basis of ANOVA. 

In figure \@ref(fig:F73) we present a few ways that subjects be combined with two two-level factors $A$ and $B$. First, we can see that factors $A$ and $B$ are crossed with each other since there are subjects, and presumably observations in each little **cell**. What would it look like for these factors to not be crossed? Well, one of the 'cells' (boxes) would be completely missing observations. If this were to happen, we would obviously lose the ability to make comparisons across groups involving the values in that cell. If your subjects factor is fully crossed with your fixed effects factors (in this case $A$ and $B$) then you have a *within-subjects* design. If you only observe certain subjects for specific combinations of fixed effects, you have a  *between-subjects* design. Finally, we see a new possibility, a **mixed design**. In a mixed design, one or more factors are between-subjects, and the others are within-subjects. In the example below, $A$ is between-subjects but $B$ is within-subjects. An example of a mixed design would arise if, for example, we wanted to run our coffee and reading time experiment on multiple first languages. Each subject could be tested at both coffee and decaf, making that factor within subjects, however, each subject would only be observed within a single first language group, making that factor between subjects. 
...
\sigma \sim N(0,12) \\
\sigma_L \sim N(0,12) \\
\sigma_{A \colon L} \sim N(0,12) \\
\sigma_{G \colon L} \sim N(0,12) \\
\sigma_S \sim N(0,12) \\ 
\nu \sim gamma(2, 0.1) \\ 
R \sim \mathrm{LKJCorr} (2)
\end{split}
...
In plain English (ignoring the priors) this says:

> "We expect apparent height to vary according to a t distribution with a trial-specific mean parameter. That mean varies based on the apparent age and gender of speaker, listener dependent intercepts and age and gender effects, and listener dependent intercepts. Speaker random intercepts were drawn from a univariate normal distribution, and the listener random effects were drawn from a three-dimensional normal distribution." 
...
We now have two non-Intercept 'Population-Level' effects: `A1` and `G1`, representing the categories 'adult' and 'female' respectively. Remember that since we used sum coding and have only two levels, the effects for the groups that are not represented ('child', 'male') are just the opposite sign of the groups that are represented. Below we calculate the mean apparent height of the four groups, and find the average of the four groups. We can see that this is very similar to our intercept. Next we find the average apparent height for levels of adultness, find the difference between these and divide it by two. We also do the same for the levels of age. We can see that these values are again reasonably similar to the estimated `A1` and `G1` predictors, which represent the same information.  
...
Below we compare our recreated group means with our average height judgment for each group. There is a reasonably close match, however there also appear to be some important mismatches. For example, the height of boys is overestimated by the model and the height of men is underestimated by the model. This suggests that our model is potentially missing important information about our data.

```{ F74b, echo=FALSE,fig.height = 3, fig.width = 8, fig.cap = "(left) Comparison of the four groups (middle) Comparison of productions based on whether the speaker is an adult (right) Comparison of all productions based on whether the speaker is male."}

################################################################################
### Figure 7.5
################################################################################

brmplot (means_pred, omit = 1, ylim = c(143,180), lab = c('b','g','m','w'), col = cols[2:5])
points (tapply (height_exp$height, height_exp$C, mean), pch=4,cex=2,lwd=2, col = cols[2:5])
```


## Linear and posterior prediction

So far we've been working with very simple models and so we haven't worried very much about how well they 'fit', or represent our data. Our reconstruction of the group means above suggests our current model may have some issues, and that we should be concerned about its ability to accurately capture the patterns in the data. One way to consider how well our model represents our data is to consider what kind would be *generated* by the relationships represented in our model. So far we've only been discussing modeling in one direction, from data to abstract, reduced representations. However, it's also worth considering going in the other direction, from the abstraction back towards data. Using your model in this way is called **prediction**, and you can use prediction to evaluate the fit of your model. The discussion of prediction presented here will involve dependent variables that do not require a link function (discussed in chapter X). Some of the details discussed here, in particular those involving expected values, do not apply to models that require a link function.

All of our models so far have contains lines like these. These lines tell our models that: 1) We expect our data to be normally distributed given some mean and standard deviation, and 2) The mean for a given trial can be predicted using some combination of the independent variables. 
...
The value of $\mu$ of above is called the **linear predictor**. When we use normal (and t?) distributed models, the linear predictor represents the expected (average) value of the dependent variables for that trial. Since the linear predictor is the value you expect for a trial, it's obvious that there should be a reasonable correspondence between the linear predictor and your dependent variables. If what your model tells you to expect is not like what you actually observe, then what good is the model? 
...
One shortcoming of the linear predictor is that this does not incorporate error. As a result, the linear predictor is a constant for any single combination of dependent variables, and undersells the amount of variation that can be expected in a given data set. The **posterior predictive distribution** is the distribution of possible data given our parameter estimates and probability model. If we assumed a normal distribution and also estimate a $\sigma$ parameter then our posterior predictive distribution is the distribution of unobserved values of $\tilde{y}$ as in \@ref(eq:77).
...
In both cases the functions return the four-column data frames with as many rows as the number of observations in the data. These data frames contain columns representing the mean, standard deviation and the upper and lower 95% credible intervals for each one of our data points. The reason we can get intervals around our predictions is because our model has a different prediction for each set of posterior samples. This means that if we have 4000 samples we actually have 4000 slightly different models and 4000 slightly different predictions. So, in addition to information about the average prediction, we get information about expected variation around these estimates.

Below we see (summarized) the data frame representing our linear predictors. You can also set `summary=FALSE` to get the individual predictions. In this case you get an individual set of predictions for every set of posterior samples for your parameters. So, if you have 1500 data points and 4000 posterior samples, the unsummarized output of these functions would be a matrix with 4000 rows and 1500 columns.  

```{r, cache = TRUE, collapse = TRUE}
head (y_lin_pred)
```

Below we see the data frame corresponding to our posterior predictions. Note that although the means are very similar, the posterior predictions are noticeably wider. This is because they incorporate error. Another difference between the linear and posterior predictions is that the linear predictors are fixed for a given set of posterior samples. In contrast, every time you run the command `predict(model)` you will get slightly different results (again because these incorporate random error). 

```{r, cache = TRUE, collapse = TRUE}
head (y_post_pred)
```

There is a parameter for the `predict` function, `re_formula`, that is set to `NULL` by default. When this happens your predictions are made using the complete model formula. For example for our last model this means our posterior predictions reflect this model formula:
...
Below we create sample from the posterior distribution and sample individual, unsammarized data. We do this for our full model and for a model that makes only fixed effects predictions. 
...
We can think of a single effect representing a difference between groups/conditions/levels as a slope. For example, in the left panel in figure \@ref(fig:F76) we plot mean apparent height for apparent females and males at x-axis locations 0 and 1. The difference in the group means is 5 cm (females 160 cm, males 165 cm). As a result, the line formed by joining these groups has a slope of 5 (i.e., it rises 5 cm from 0 to 1, for a slope of 5/1). We can use any arbitrary x axis distance to calculate slopes, as long as we are consistent. However, there are obvious practical advantages to choosing to calculate these slopes over the arbitrary 'distance' of 1. The plot highlighting the effect for adultness on apparent height is a 'main effects' plot. You may have heard things like "the analysis showed a significant main effect for so and so...". Main effects are the average effects for one predictor averaged across everything else. Saying 'averaged across everything else' basically means we are ignoring everything else in our model. A person looking only at the left plot in figure \@ref(fig:F75) would not realize our data also investigates the effect of adultness. We have 'erased' the differences in adultness by averaging across all levels of that factor. Another way to think of main effects is that they are 'marginal' effects, the overall, average, unconditional difference. So, if someone asks you "whats the average difference in apparent height between males and females?" and you can respond "about 5 cm". As noted in the previous chapter, sometimes the answer is not so simple, and it starts more like: "well... it depends". Interactions represent situations like these, where the effect of one variable depends on, or is *conditional* on, the value of some other variable. 

To understand interactions we need to talk about **simple effects** or **simple main effects**, the effects of a factor at one specific level of another factor. Basically, main effects show you the effect for a factor and simple effects show you the effects of the factor *depending* on the value of other things. For example, we can break up the effect of gender above into two effects, one for apparent adults and one for apparent children. In doing this we have split the singe (main) effect into two (simple main) effects. The simple effects of gender conditional on apparent age are presented in the second plot in figure \@ref(fig:76). Imagine what would happen if we began with the lower (blue) line and added *only* a main effect for apparent age to the blue line. Since a single number would be added to both points on the line, the line would *slide* up to y-axis, however, the green and blue lines would necessarily be parallel. We can see that these simple effects of gender given age have different slopes. In fact, the slope (effect) across genders is only 1 cm for children while it is 10 cm for adults. In order to make the green and blue lines *not* be parallel, we need to add different values to each side of the line segment. In other words, the effect for age (the difference between the lines) would need to differ based on whether apparent gender is male or female. This would not longer be a main effect but an *interaction*. As a result, when we our simple effects plots show lines that are not parallel, that means there may be an interaction in our data. 
...
Let's consider the main and simple effects for perceived adultness. There is a 24 cm decrease in average apparent height based on the perceived adultness of the speaker. So, there is a large main effect for adultness. In the fourth plot in figure \@ref(fig:76) we see that the simple effect for adultness at across levels of gender are not parallel. This tells us there is an interaction between age and gender. We can think of this interaction in two ways: 1) As the difference in slope between the two lines, 2) as the difference along the y-axis for each set of line-segment end points. For example, we can say "there is a larger effect for age on apparent height for male speakers relative to female speakers". This would focus on the difference in slope between the lines. In contrast, we might also say "there is an effect for apparent gender for adult speakers but not for child speakers". This would focus on the difference in the respective end points. Anyone who is able to make such a statement based on the information in the plot above *understands* interactions, whether or not they know how to relate this concept to the mathematical formalisms used to implement this concepts in regression models. The idea of conditional effects (interactions) *feels* complicated, but it is something we all understand intuitively. How much progress will a person learning a second language make in a year? What if I told you that one group of speakers is 3 and the other is 65. You *know* that makes a difference, which is to say, that you *know* there is an interaction between the effect of time spent learning a language, and the age at which the learner begins. 

The second and fourth plots in figure \@ref(fig:76) are sometimes called **interaction plots**. Knowing how to interpret interaction plots is extremely useful because they help you quickly understand the nature of any interactions in your data. Interaction plots show you the simple main effects of one predictor across the levels of the other. They do this by having one factor vary across the x-axis and using different lines for the levels of the other. Note that the same information is presented in the two interaction plots in figure \@ref(fig:76), it is just presented differently in each case. The slopes in the second plot are the differences between line endpoints in the fourth plot, one is small and one is zero. The differences between the line endpoints in the second plot are the slopes of the fourth plot, both are large but one is larger. 

Before moving on from interactions, we need to discuss the relationship between interactions and the crossing of factors. You cannot estimate interactions of factors are not crossed. This is kind of obvious if you think about it. Interactions are conditional effects, and crossing means you observed all combinations. If you did not observe all combinations, you are not in a position to talk about how effects vary for specific combination of factors, which is what interactions are. This also applies to interactions fit using adaptive partial pooling, i.e. random effects. For example in the last chapter we fit a model with random listener-dependent effects for apparent age. We were only able to do this because we had data for each listener for each apparent age. If, for example, listener 11 only ever identified speakers as children, how could we possibly know the listener-dependent effect for age for this listener? So, it is extremely important to be sure to only include interactions in your model when these predictors are *crossed*, or your model may run into problems. 

## Investigating interactions with a model

Above we made posterior predictions of our data using our model, with and without the inclusion of the speaker random effects. We presented these as box plots in figure \@ref(fig:75), and we present these as interaction plots in figure \@ref(fig:77). We can see that the predictions made by using our fixed effects are systematically wrong, and that the problem with our predictions is that the lines should not be parallel. As we've just discussed, in the absence of interactions, interaction plots contain only parallel lines. Well, since our model (`model_both`) does not include interaction terms it cannot represent interactions, and so is only capable of producing predictions that result in parallel lines. This means it is not capable of representing the pattern in our data using only fixed effects. We can see that when random effects are included, prediction is quite good. This is not surprising since the listener and speaker-specific intercept adjustments allow for deviations from the fixed effects predictions to still be captured by the model. Despite this, we should be concerned about the problems with our fixed effects predictions. Our model is a little universe we made up, and it only includes the information we included in it. This 'universe' only contains parallel lines because we only included that capability. So, the fact that your model generates parallel lines does not in any way 'prove' that the lines are parallel, because they were bound to be. In order to properly investigate whether the lines *should* be parallel, the model must be built in a way that allows it to represent the interactions in our data. 
...
Our full model specification is presented in \@ref(eq:78). The model now includes an *interaction* term ($A \colon G$) that can help explain variation that cannot be explained by the independent effects of adultness and gender. This interaction term helps us model the *conditional* effect of one predictor given the other. Our model also actually contains a three-way interaction as well ($A \colon G \colon L_{[L_{[i]}]}$), though it may not seem that way to us superficially. This is the 'random' effect of the interaction between $A$ and $G$, `(A*G|L)` in our model. This is a three-way interaction because it represents the interaction between the two-way interaction of $A$ and $G$ with our listener factor. 
...
A \sim N(0,12) \\
G \sim N(0,12) \\
A \colon G \sim N(0,12) \\
\sigma \sim N(0,12) \\
\sigma_L \sim N(0,12) \\
\sigma_{A \colon L} \sim N(0,12) \\
\sigma_{G \colon L} \sim N(0,12) \\
\sigma_{A \colon G \colon L} \sim N(0,12) \\
\sigma_S \sim N(0,12) \\ 
\nu \sim gamma(2, 0.1) \\ 
R \sim \mathrm{LKJCorr} (2)
\end{split}
...
Below we fit the our model now including an interaction term. Remember that the line `set_prior("student_t(3, 0, 12)", class = "b")` sets the prior for all non-intercept 'Population-Level' predictors (i.e. fixed effects). This our course includes all of our main effects predictors but also all of our interaction terms. 
...
And see that we have estimated four standard deviation terms for our listener-dependent terms. These correspond to $\sigma_{L}$, $\sigma_{A \colon L}$, $\sigma_{G \colon L}$, and $\sigma_{A \colon G \colon L}$. We have also estimated six correlation terms. Recall that our random effects are drawn from a multivariate normal distribution. We have four terms which means this distribution is associate with a 4x4 covariance matrix with 16 correlation terms. Of these, four (on the main diagonal) equal one and can be ignored. This leaves 12 terms. Of these only half are unique because the correlation of $x$ and $y$ equals the correlation of $y$ and $x$. This is how we end up with six estimated correlations for our four listener-dependent predictors. 

We can now focus on the fixed effect (`Population-Level Effects`), of which there are four: An intercept, two main effects terms and an interaction. Why is there only a single interaction? Basically for the same reason that we can only estimate a single age and gender effect (i.e., linear dependence, discussed in section X). The number of lvels of a factor that you can estimate is generally one fewer than the number of levels in the factor. For interaction terms, the number of parameters you can estimate is equal to (number of levels of factor A - 1)x(number of levels of factor B - 1) for two factors. Since each of our factors have two levels, we can only estimate one parameter, (2-1)x(2-1)=1. The interaction term is just another element of your prediction equation (i.e., $\mu + x_1+x_2...$) intended to help explain variation that can't be predicted by the independent effects of the other predictors in the model. If there is no interaction in your data, then the value of the interaction terms will be zero and your model will look just as if you had not even included the interaction at all. A look at the model output above indicates that our interaction is much smaller than the age effect, but not much smaller than the gender effect. This suggests it has the potential to influence our conclusions about gender more than our conclusions about age. 

Ok, so we have a single interaction term, but what does it *mean*? The `A1:G1` term can be read aloud as "the effect of `A1` given (conditional on) the level of `G1`". Just as our models cannot estimate `A2` because `-A1 = A2` they cannot estimate `A2:G1` because `-A1:G1 = A2:G1`. In fact, `-A1:G1 = A2:G1 = -A2:G2  = -A1:G1`, which is precisely why you can't estimate more than one interaction for this model. Basically, you start with `A1:G1` and every time you change `A1` or `G1` to `A2` or `G2` you flip the sign of the term. If you flip both terms you flip the sign twice, meaning you do not change it. Why does this happen? Think back to the interaction plots presented in figure \@ref(fig:76). We discussed interactions as difference in slopes. If the average slope is 5 and one group has a slope of 4, obviously, the other slope *must* be 6. So, the slope interaction terms here would need to be -1 and 1, meaning both can't be estimated. Relatedly, it is obvious that the distances between the ends on the line segments are totally dependent on the differences in the slopes. Since the differences between the line segments reflect the effects of the *other* predictor in the interaction plot, we en up with one single value accounting for what seems like four possible differences in our data. 

Figure \@ref(fig:F78) presents an example of how our interaction plot can be built up from the components in our fixed effects above, and is very useful for understanding the geometry of main effects and interactions. In (a) we see that the intercept lifts up a line from 0 to the level of the overall mean apparent height of 160 cm. Then, the effect for `A1` (adult, +10 cm) and `-A1` (children, -10 cm) are added to this value, causing the separation between the lines indicating a 'main effect' for adultness. Notice that the lines are parallel with respect to each other, and also parallel to the x-axis. This is because we have not added either an effect for gender (indicated as a slope along the x axis), or an interaction between gender an adultness. In (b) we see the addition of the gender effect: `G1` (female, -2.74 cm), and  `-G1` (male, 2.74 cm). Notice that the effect is added to one end of each line segment and subtracted from the other end of the line segment. The result is the same non-zero slope for both lines (i.e. no interaction yet). Panel (c) shows the addition of all of the effects shown in the top row, the result of a model that includes both main effects both no interaction. In (e) we add the gender interaction `A1:G1`, which was -1.45 cm. Notice that the interaction effect has different signs when added to the same line, and also for each end of each line segment. This is necessary in order to maintain the same average value for any given line segment, and therefore the fixed value of the main effect.  

```{r F78, echo = FALSE, out.width = "100%", fig.cap = "An interaction plot built up from its individual components."}
...
Recovering the predicted group means using the model coefficients is straightforward, but a bit tedious: We must now either add or subtract the value of the interaction term (`A1:G1`) from each group. We can easily determine which to do for this model because the sign on the interaction term is the product of the signs on the relevant 'main effects' terms. Another trick is to remember that a single sign flip changes one side of the interaction while a double sign flip changes both. For example, `A1:G1` represents the first level of the age and gender factors, adult females. If we flip the sign to `-A1:G1`, this now represents both adult males *and* female children. If we flip the sign twice `A1:G1` represents adult females. Below, we use the `hypothesis` function provided in `brms` to reconstruct expected group means given the fixed effects above. The fourth hypothesis we are testing below is the simplest to understand, so we will start there. This hypothesis asks whether the sum of the `Intercept`, `A1` (the effect for adultness), `G1` (the effect for femaleness), and `A1:G1` (the interaction of adults and female). Because of the specific combination of parameters values we combined, this results in the comparison of the 'woman' mean height to zero. The first hypothesis we are testing below says `Intercept + -A1 + -G1 + A1:G1 = 0`. Since we have flipped the sign on `A1` and `G1`, these now correspond to the effects of 'child' and 'male'. Since both `A1` and `G1` are negative, we flip the sign on the interaction term twice and it remains negative. Note that for all of our hypotheses below, the sign on the interaction terms always depends on the signs of the corresponding main effects terms. By comparing our recovered group means to we can see that the inclusion of an interaction term allows our model to capture group averages more accurately than the model without intercepts.
...
Below we print the estimates of the 'fixed' effects in the model so we can focus on those. If you fit a model like this and are having trouble interpreting it, we would really encourage you to write the coefficient values down and try to recreate expected effects them using pen and paper, focusing on the decomposition of values provided by the regression model. An example of this process will be given below.  

```{r, collapse = TRUE}
brms::fixef (model_interaction)
```

For example, we can recover the simple effects for age at the levels of gender. The average apparent height across all groups is 160 cm. There is an age-based 10 cm deviation from the intercept, meaning the difference between adults and children is 20 cm (10 * 2). This means that, overall, the adults and children averages are about 170 and 150 cm (160 ± 10). However, the `A1:G1` interaction is -1.5 cm. This means that when the speaker was female (`G1`), the affect for adultness decreased from 10 cm to 8.5 cm (10 - 1.5, `A1+A1:G1`), meaning the difference across adults and children was 17 cm (`2*8.5`). In contrast, while when the speaker was a male (`-G1`), the affect for adultness increased from 10 cm to 11.5 cm (10+1.5, `A1-A1:G1`), meaning the difference according to age group was about 23 cm. We could instead consider the simple effects for gender given age. There is a gender-based 2.7 cm deviation from the intercept, meaning the difference between adults and children is -5.4 cm (-2.7 * 2). This means that, overall, the female and male averages are about 165.4 and 154.6 cm (160 ± -5.4). However, the `A1:G1` interaction is -1.5 cm. This means that when the speaker was an adult (`A1`) the affect for gender increased in magnitude from -2.7 cm to -4.5 cm (-2.7 - 1.5, `G1+A1:G1`), resulting in a difference between groups of about 9 cm. Finally, when the speaker was a child (`-A1`), the effect for gender decreased in magnitude from -2.7 cm to 1.2 cm (-2.7+1.5, `G1-A1:G1`), resulting in a difference in groups of about 2.4 cm based on apparent gender. 
...
We can asses model fit for the model including interaction terms by making more posterior predictions with our new model. We will focus on the average predictions for each data point since our objective is to see to what extend the fixed effects can represent the patterns in the data. 
...
```{r F79, fig.height=5,fig.width=8,fig.cap="Interaction plots showing comparing our f0 data to posterior predictions, with and without interaction terms (neither contains Random Effects)."}
...
To some extent the difference may appear to be smaller than it is because our random effects do a good job of explaining listener and speaker-dependent deviations from the expectations set out by the fixed effects. To quantify how well our models fit our data we can use **Bayesian $R^2$**. The account of Bayesian $R^2$ presented here is a summary of the proposal outlined in Gelman (cite). $R^2$ is a measure of model fit that breaks down the total variance in your dependent variable into two parts: 1) The variance that your model can explain, and 2) the variance that your model *cannot* explain, the residual. This is shown in \@ref(eq:79). The discussion of $R^2$ to follow assumes that your model involves either Gaussian or t-distributed errors and a continuous dependent variable. 
...
To make $R^2$ a *Bayesian* $R^2$, we need to incorporate the posterior distribution of parameter values into our estimation, as shown in \@ref(eq:711). For every posterior sample of parameter values $s$ (from $S$ total samples) we calculate the variance in the predicted values ($\hat{y}$) of the dependent variable for our $N$ data points ($V_{n=1}^N \; \hat{y}^s_n$) and the variance of the residual error for our $N$ data points ($V_{n=1}^N \; \hat{e}^s_n$).
...
The residual error is the difference between predicted and observed values for each data point, shown in \@ref(eq:711).
...
The `brms` package includes a function called `bayes_R2` that helps you calculate Bayesian $R^2$ for `brm` models. However, we are going to use the `bayes_r2` function from the `bmmb` package instead because this function allows for control over the fixed effects included in the prediction. Below we calculate $R&2$ for the models we fit this chapter. 
...
We can inspect the output and see that they have very similar values. 
...
Now a comparison suggests a more substantial difference between the models. 
...
Figure \@ref(fig:F710) compares our $R^2$ values with and without random effects. These values sort of just formalize what we saw in our interaction plots in figure \@ref(fig:F79), that the interaction term helps model fit a lot of we consider only the model fixed effects structure. 
...
brmplot (pairwise_diffs,labels = c("b-g","b-w","b-m","g-w","g-m","w-m"))


```

Figure \@ref(fig:F711) also presents the difference between all four apparent speaker groups (boys, girls, women, and men). The difference between group means is calculated by extracting the samples from the group means we reconstructed in section 7.X (`means_pred_interaction`). The result of this is a matrix with four columns, each representing a group effect. The individual posterior samples differ along rows. To find the difference between group means between each pairs of groups, we subtract the columns representing the group effects and inspect the distribution of the differences. This process is shown below and the distribution of pairwise group differences in presented in figure \@ref(fig:F711).
...
We can think about these differences in the context of our type M and S errors. All of the group differences (save for boy-girl) have values of about 10 cm or greater, which is a substantial difference in human height. In addition, the credible intervals around these differences are small relative to their magnitudes. From this we can conclude that these differences are unlikely to *really* by the opposite sign (a type S error). In addition, since the group differences (save for boy-girl) are mostly large and the credible intervals do not go anywhere near zero, these differences seem unlikely to actually be some small and insignificant value near zero (a type S error). Ok, so what about the boy-girl difference? We present this difference in more detail below.    

```{r, collapse = TRUE}
# summary 
pairwise_diffs_summary[1,]

# some relevant quantiles, 2.5%, 25%, 50%, 75%, 97.5%
