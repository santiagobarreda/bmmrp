...
For example, for the roll of a single die the possible outcomes are the integers from 1 to 6 and each has a 1/6 chance of occurring. To calculate the expected value we sum 1/6 (i.e. $y_{[1]}P(y_{[1]})=(1)(1/6)$), 2/6, 3/6, 4/6, 5/6, and 6/6, resulting in 3.5. So, the expected value of a roll of a single die is 3.5. This is why the game of craps involves rolling two dice and centers on whether a player rolls a 7: This is the most probable outcome when one rolls two dice since the expected value of a single die is 3.5. Continuous variables have an analogous formula defining the expected value with an integral and a probability density function ($f(y)$), as seen below. 
