...
The mean of a normal distribution represents the **expected value** of that variable. For discrete variables, the *expected value* is the sum of the values ($y$) multiplied by the probability of observing each value ($P(y)$):
...
Note that the sample mean is an estimator of the expected value. In the case of the sample mean, the division by $N$ reflects the fact that we treat each observation as equally likely, estimating its probability as $\frac{1}{N}$. So, if we were to replace $P(y)$ with $\frac{1}{N}$ above, equation \@ref(eq:2-3) starts to look a lot more like \@ref(eq:2-2). Continuous variables have an analogous formula defining the expected value with an integral and a probability density function.
...
Below we see that we can use a `for` loop to recreate Equation \@ref(eq:2-3). However, we don't manage to exactly recreate the output of the `var` (variance) function included in R. 
...
It's fairly important to more or less understand the 

\@ref(c2-inference-and-likelihood)
...
We will pick up where we left of in considering the likelihood of $\mu$ given some data, presented in equation \@ref(eq:2-13), presented again as \@ref(eq:2-13).
