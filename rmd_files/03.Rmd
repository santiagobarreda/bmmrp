...
[@@ SB integrate this with figure and equation 3.12 bettre. expand more.]
...
legend (176,.9,legend=c("Prior","Likelihood","Posterior"),lwd=4,lty=c(1,1,4),
...
[@@ SB - mention that this is P(mu|y) basically]
...
By default the distribution of residuals is presented, but you can get the individual estimates themselves by calling `residuals(model,summary=FALSE)`. [@@ SB- expand on this. why do you get so many residuals, what is being summarized, etc.]
...
When we got our posterior samples with the `get_samples` function, in addition to our mean and standard deviation estimates we got an estimate of something called `lp__`. This is a measure of the (unnormalized) **log posterior density**, basically, the overall posterior probability of your model parameters given your data. 
...
In \@ref(eq:3-12) we presented the posterior probability of a $\mu$ parameter given your data and priors. We can generalize this to encompass all of our model parameters, represented by $\theta$ in \@ref(eq:3-22). Now, \@ref(eq:3-22) represents the posterior probability of all our model parameters given our data.
...
If we log-transform both sides of \@ref(eq:3-23), we get the log-posterior density, shown in \@ref(eq:3-24) and including some constant $C$. [@@SB explain C]
