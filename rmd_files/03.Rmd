...
Abstract:
This chapter introduces the concept of a regression model and explains what makes a regression model 'Bayesian'. Posterior probabilities are introduced as is `brms`, an R package that can be used to fit Bayesian regression models using the Stan programming language. After this, an 'intercept only' Bayesian regression model is fit to the experimental data. Topics related to model fitting such as thinning, chains, and iterations are presented, as is the specification of prior probabilities. The interpretation of the `brm` model print statement is discussed, and the concepts of errors and residuals are introduced. Finally, information related to working directly with the model posterior samples is also provided, and the log prior and posterior densities are discussed.
...
1) Easy: Analyze the (pre-fit) model that's exactly like the one in this chapter, except using the data in `exp_ex` (`bmmb::get_model("3_model_priors_ex.RDS")`).
