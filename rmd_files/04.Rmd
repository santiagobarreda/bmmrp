...
knitr::include_graphics("../images/unilevelvsmultilevel.jpg")
```
...
We can use `brms` to fit a model that more closely resembles our model specification in \@ref(eq:4-9). To do this we need to use the formula given above, and we also need to specify priors for three estimated parameters: $\mathrm{Intercept}$, $\sigma_L$, and $\sigma$. We can confirm our expectations using the `get_prior` function as seen below (omitting empty columns 7-9 so it will fit on the page). 

```{r}
brms::get_prior (height ~ 1 + (1|L), data = men)[,-c(7:9)]
```
...
```{r}
colMeans (listener_intercepts)[1:8]
colMeans (listener_intercepts)[9:15]
```
...
mtext (side=1,outer=TRUE, "Apparent speaker height (cm)",line=-1.3)
```
...
```{r F4-10, fig.width=8, fig.height=2.5, fig.cap="(left) In green, the random speaker intercept estimates provided by brm. The red arrows indicate the estimates of the same provided by lmer. .", echo = FALSE}
