...
When we then use this listener vector to estimate the average height reported by each listener, the ordering of the levels is maintained. 

```{r}
# find average reported height for each listener
listener_means = tapply (men$height, L, mean)
listener_means
```

If we center the listener_means vector above, this is basically equivalent to our vector of listener effects $L_{[L]}$. 

```{r}
listener_effects = listener_means - mean (listener_means)
```

Unfortunately, R names the predictor vector using the same name. As a result, we have a predictor called `L` which represents listeners, *and*, we have a set of coefficients called $L$ in our model that represent the effects for each of our listeners. So, when you see    
...
So the model formula `height ~ 1 + ( 1 | L)` tells `brm` to build an intercept only model with a separate intercept for each listener, and that the listener intercept terms should be estimated with partial pooling. In plain English, the model in \@ref(eq:45) says:
...
This model contains one new chunk its print statement, which tells us about the standard deviation for the Intercept (`sd(Intercept)`) for the `~L` factor. The term `sd(Intercept)` in this section corresponds to $\sigma_L$, the standard deviation of listener effects ($L_{[L]}$) in the sample. 
