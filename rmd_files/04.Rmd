...
adaptive pooling, complete pooling, extremely, fixed effects, hierarchical models, homoscedastic, homoscedasticity, hyperprior, mind fallacy, mixed-effects models, multilevel model, no pooling, Partial pooling, point estimates, random-effects models, random effects, repeated data
...
For example, if we have single samples of speech from each of 50,000 adult males from Los Angeles, we may be confident that we can reliably estimate the speech characteristics of adult male speakers from Los Angeles. But what if our 50,000 samples were from only five different speakers? It's obvious that this makes the data less reliable for making inferences about Los Angeles in general, although the 10,000 observations from each speaker likely provide excellent information about the five people we measured. So, the repeated observation of a limited number of subjects may give you good information about the specific subjects observed, but it may also be less useful to make inferences about the population.
...
Figure \@ref(fig:F4-1) presents the height judgments collected for adult male speakers in our experiment, organized in three different ways. In the right panel we see the marginal distribution of the data with no differentiation made between listeners. This boxplot shows the individual observations of our vector $height_{[i]}$ around the overall mean $\mu$ with a standard distribution equal to $\sigma_{[total]}$. In the middle panel we see each listener's height judgments in a different boxplot. Each listener has a different mean value which we will refer to as $L_{[n]}$ for listener $n$, $L_{[\bullet]}$ for any individual unspecified listener, and $L$ with no subscript when referring to all listener mean values, together as a set. 

As seen in figure \@ref(fig:F4-1), each listener has a distribution of height judgments around their average value. For the sake of simplicity, we will assume that the standard deviation of these distributions is equal for all listeners, and we will call this value $\sigma_{[within]}$. Finally, in the left panel we see the distribution of individual listener averages ($L$). This distribution is similar to considering the distribution of thick bars inside each individual boxplot (although these bars represent the medians and not the means). We can call the standard deviation of this distribution $\sigma_{[between]}$, since it represents between-listener variation in average height judgments.
...
The model above estimates $\sigma_{[total]}$ ($\sigma$ in the model above) but not $\sigma_{[within]}$ or $\sigma_{[between]}$. As a result, this model treats all deviations from the mean as random and therefore acts as if systematic between-listener variation did not exist. However, it is clear that listeners *do* show consistent tendencies in the heights they tend to report, so that systematic between-listener variation ($\sigma_{[between]}$) should not be ignored, or treated as if it didn't exist. 

For example, a height response of 181 cm may be 7 cm above the overall mean, but is perfectly average for listener 6. From the perspective of the model above, an observation of 181 cm is +7 above the mean regardless of who provided it, because 164 cm is the assumed mean for all listeners. As a consequence, all variation about the mean is noise. However, if you know listener 6 provided a response, then an apparent height of 181 cm should not be surprising and would be perfectly 'on target' for this listener. So, an observation of 181 would be an error of 0 cm when provided by listener 6, considered relative to their expecte apparent height judgment. 
...
* The 'lower' level: Is the distribution of the *data* itself, our individual observations and data points. We expect that each listener will produce judgments around their specific average ($L_{[\bullet]}$), with (for the sake of simplicity) a standard deviation equal to $\sigma$ for all listeners (referred to as $\sigma_{[within]}$ above). Clearly, there may be many good reasons for listeners' height judgments to vary from their average. However, our model cannot explain this and so this is treated as 'error' by our model. 

* The 'upper' level: The distribution of *parameters* between your sources of data. In this case this is the average height reported by any given listener $L_{[\bullet]}$. The listener averages can be thought of as random variables, since we can't explain exactly why a listener average deviates from the overall average and any given listener's $L$ is unpredictable a priori. We can assume that this upper-level distribution is also normal with a mean of $\mu_L$ and a standard deviation equal to $\sigma_L$ (referred to as $\sigma_{[between]}$ above).  

A **multilevel model** is a special kind of regression model that is able to simultaneously model random variation at multiple levels. A multilevel model fit to our height judgments would be able to estimate the values of $L$ for all listeners, the variation in listener means ($\sigma_L$), and the random within-listener variation ($\sigma$), all at the same time. The structure of such a multilevel model can be compared to the structure of the 'unilevel' model we fit to our height judgments in Chapter 3 (and seen in \@ref(eq:4-1)). In our 'unilevel model' there is only one level and one distribution: The data distribution. In the simple multilevel model we just introduced, there are two levels: The distribution of *parameters* 'above', and the distributions of *data* 'below'. 

In the 'unilevel' model in figure \@ref(eq:4-2), all of the variation in $y$ from the average is error and constitutes the residual: Unexplainable random variation. In contrast, in the multilevel model in the same figure variation between $L$ and the mean is random, but is not treated as *error*. Instead, this can be thought of as potentially *systematic* random variation. For example, if we replicated the experiment with the same listeners, procedure, and stimuli, we might expect that the values of $L$ might be about the same. As far as the multilevel model is concerned it is only the within-listener variation that is random error, and this constitutes the residual.

[@@ SB - noah check out above and below, its been moved and edited]
...
One thing to note is that if there is no between-listener variation, i.e. if $\sigma_L=0$, the the multilevel model above converges on the unilevel model. This is because if $L_{[\bullet]}$ is equal for all listeners, then $L_{[\bullet]} = \mu_L = \mu$. Since the multilevel simply estimates values of $L$ and $\sigma_L$, the outcome $\sigma_L=0$ is within the realm of possibility. So, fitting a multilevel model allows one to investigate the very existence of systematic variation in parameters across research units (e.g. listeners) but not does not necessarily impose this on the data.

## Strategies for estimating factors with many levels {#c4-strategies}

Our models in the previous chapter only included an intercept. However, it seems that our model should include coefficients that would allow us to model systematic variation in apparent height judgments across different listeners. R treats nominal, categorical predictors as *factors* and assumes that each different label represents a different group. Each value of a factor is called a *level*. Although we can indicate listeners using numbers, we are not treating this predictor as numerical or otherwise quantitative (i.e., Listener 1 isn't one less than Listener 2). Instead, we will treat listener (`L`) as a factor, and the individual listeners in the experiments are its levels. As far as our models are concerned, participant/speaker/subject/listener has no special status as a predictor and it is just a factor with many levels. 
...
When you see terms like these $L_{[L_{[i]}]}$ in your models, this is nothing more than a more compact way of representing the same model structure shown in \@ref(eq:4-2). For example, in the code below we use the equivalent notation in terms of our `listener_effects` vector, where we show the listener affect associated with the 300th trial in our data.

```{r, eval = FALSE}
# observation 300 was provided by listener 7
L[300]

# the listener effect for observation 300, listener 7
listener_effects[L[300]]

# the listener effect for listener 7
listener_effects[7]
```
...
Gelman and colleagues (cite) distinguish three general ways that parameters related to levels of a factor can be estimated: 1) *Complete pooling*, 2) *No pooling*, and 3) *Partial pooling*. The distinction between complete, no, and partial pooling is made for each factor in a model. So, one factor may be estimated using partial pooling while another may be fit using no pooling. This makes understanding the distinction between these approaches very important.  
...
```{r F4-5, echo = FALSE, fig.width = 8, fig.height = 3, fig.cap = "Comparison of real and simulated apparent height data. Each box represents data from one listener, the horizontal line represents the mean of the listener means. Which is the real data?"}
...
Below we make two datasets that are 'incomplete' with respect to the variation in our data. The first contains the intercept and noise only, the second contains the intercept and listener effects only.
...
In figure \@ref(fig:F4-6) we compare these 'incomplete' datasets to the full simulated data. The top row contains only error ($\sigma$). As a result, height judgments vary around the intercept, but there is no listener to listener variation. This is what that data would look like if there is no variation in means across listeners, as is assumed by the 'unilevel' (complete pooling) model in figure \@ref(fig:4-2). However, notice that each little box is not centered at zero. Although we have not added any variation in listener means, a small amount of this appears to be present in our data. Although the error distribution is centered at zero, the small number of errors added to the true listener mean are **extremely** unlikely to add up to exactly zero. So, error contributes some unknown amount to each listener's sample mean estimate. This makes estimations of the *real* listener effects impossible in practice. 

```{r F4-6, fig.width = 8, fig.height = 6, fig.cap="(top) Simulated data containing within-listener variation but no between-listener variation. (middle) Simulated data containing between-listener variation but no within-listener variation. (bottom) Simulated data containing both within and between-listener variation in apparent height.", echo = FALSE}
...
In the middle plot, the figure shows only between-listener variation ($\sigma_{L}$) but no within-listener variation (i.e., no $\sigma$). Now listeners vary from the intercept, but they do not vary within themselves. The final plot is the combination of the variation in the top two plots and shows the final simulated data: The sum of the intercept, the within-listener variation and the between-listener variation.

As seen in figure \@ref(fig:F4-6), the model without actual listener variation is unlikely to generate the substantially-different listener boxplots we see in our data through noise alone. We can try another simulation, identical in all respects except we set the between-listener variance to zero, and set the noise variance to 7.8 (as in our complete pooling model). Below we generate 10,000 simulated data sets and record the standard deviation of the listener means for each one. These values will be stored in a vector called `sigma_L_rep` since we are simulating values of $sigma_L$ for our complete pooling model.
...
We've mentioned that multilevel models can result in *shrinkage*, that is, in effect estimates that are smaller in magnitude (i.e. closer to the mean) than their no pooling counterparts. In the code below we extract the posterior mean estimates of our listener and speaker random effects (i.e., $L$ and $S$). We also calculate the no pooling estimates for the listener and speaker averages, and center these so that they will resemble their corresponding effects. 
...
L_np = tapply (men$height, men$L, mean)
S_np = tapply (men$height, men$S, mean)
# and now they are centered
L_np = L_np - mean (L_np)
S_np = S_np - mean (S_np)
```

In the top row of figure \@ref(fig:F4-8) we see a comparison of the shrinkage estimates of the speaker and listener effects compared to the no pooling estimates. We can see that the Bayesian shrinkage listener effects are nearly identical to their no pooling counterparts. In the bottom row we see the difference between our two estimates plotted against the no pooling estimates. In this row we see that the degree to which an estimate is shrunk towards the mean depends on how much it deviates from the mean.   
...
plot (L_np, L_hat, xlim = c(-10,10), ylim = c(-10,10),
      pch=4, lwd=3, col=bmmb::cols,cex=2.25,xaxt="n",xlab="")
grid()
abline (0,1,col=2,lwd=2)
abline(h=0,v=0,lty=3,lwd=2)
points (L_np, L_hat, pch=4,lwd=3, col=bmmb::cols,cex=2.25)

plot (S_np, S_hat, xlim = c(-12,12), ylim = c(-12,12),
      pch=4, col=4,cex=2.25,xaxt="n",xlab="")
grid()
abline (0,1,col=2, lwd=2)
abline(h=0,v=0,lty=3,lwd=2)
points (S_np, S_hat, pch=4,lwd=3, col=bmmb::cols,cex=2.25)

plot (L_np, L_hat-L_np, xlim = c(-10,10), ylim = c(-5,5),
      pch=3, lwd=3, col=bmmb::cols,cex=2.25)
grid()
abline(h=0,v=0,lty=3,lwd=2)
points (L_np, L_hat-L_np, pch=3,lwd=3, col=bmmb::cols,cex=2.25)

mtext (side = 1, text = "L_np", line=2.5)

plot (S_np, S_hat-S_np, xlim = c(-12,12), ylim = c(-5,5),
      pch=3, col=4,cex=2.25)
grid()
abline(h=0,v=0,lty=3,lwd=2)
points (S_np, S_hat-S_np, pch=3,lwd=3, col=bmmb::cols,cex=2.25)

mtext (side = 1, text = "S_np", line=2.5)
```

Figure \@ref(fig:F4-9) shows the effect of shrinkage for the listener and speaker effects: Individual effects are 'pulled' towards the overall mean at zero. There are two reasons for the listener effects exhibiting less shrinkage than the speaker effects. First, we have 45 observations for each listener but only 15 for each speaker. As a result, the likelihood functions for our listener effects will be much narrower than those of our speaker effects. This has the effect of making our listener parameters more resistant to the effects of the prior, and therefore more resistant to shrinkage (see sections \@ref(c3-posterior) and \@ref(c2-chars-of-likelihoods-2) for more information). Second, a larger proportion of the speaker effects are close to the average, with only a relatively small number exhibiting large deviations from the average, whereas the listener effects are less concentrated near the average. The large "mass" of speaker effects near the average causes more shrinkage, particularly on the small number of more extreme values.
