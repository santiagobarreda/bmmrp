...
- these apply not to models but to individual factors. we can chose one or antoerh for each factor in a model.
...
We've mentioned that multilevel models can result in *shrinkage*, that is, in coefficient estimates that are smaller in magnitude than their maximum likelihood counterparts. 
...
epsilon = rnorm (45 * 15, 0, 7.78)
height_rep = Intercept + L_L[L] + epsilon
fake_data = data.frame (height_rep, L)

sd (tapply(height_rep, L, mean))
```

```{r,eval = FALSE}
# Fit the model yourself
set.seed (1)
fake_multilevel_model =  
  brms::brm (height_rep ~ 1 + (1|L), data = fake_data, chains = 4, cores = 4,
             warmup = 1000, iter = 3500, thin = 2,
             prior = c(brms::set_prior("normal(176, 15)", class = "Intercept"),
                       brms::set_prior("normal(0, 15)", class = "sd"),
                       brms::set_prior("normal(0, 15)", class = "sigma")))
