...
[@@ Go over whole start, see if it makes sense, really really drill home the two Ls in the equation. possibly merge figures 41 and 42 or at least talk about both together more. ]
...
When you see terms like these $L_{[L_{[i]}]}$ in your models, this is nothing more than a more compact way of representing the same model structure shown in \@ref(eq:4-2). If your data contains a predictor called $L$ whose first values are 2, 3, 3, and 2, then the $L$ parameter in above will be $L_{[2]}$, $L_{[3]}$, $L_{[3]}$, and $L_{[1]}$ for the first four trials. For example, in the code below we use the equivalent notation in terms of our `listener_effects` vector, where we show the listener affect associated with the 300th trial in our data.
...
[@@ SB here explain that pipe is given, that coefficients on left estimates CONDITIONAL on values of levels on right. this is discussed further in section c6-variation-sources]
