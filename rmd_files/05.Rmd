...
## Refitting the model with sum coding {#c5-refittin-sum}
...
Regression models try to break up the value of the dependent variable into different components. This is why effects are expressed in terms of differences to some reference value. For example, imagine we say that a speaker's apparent height is 160 cm, and under some other condition their apparent height is also 160 cm. Didn't we just say that this condition has no *effect* on their apparent height? To say that this condition has no effect on apparent height is to say, at least in part, that it is associated with no difference. On the other hand something that *is* associated with a difference in apparent height *can* be said to have an that variable. As a result, we can express the *effect* of something in terms of the difference it is associated with. For example, we can say that under so and so conditions a person will tend to sound 17.5 cm shorter, relative to some reference value. More generally, we can think of any variable as the sum of a bunch of independent *effects*. 

We want to be sure to not imply that effects are necessarily *causal*. For example, you could guess someone's age from their height but height obviously does not cause age. Despite this, if you built a model that predicts age based on height you might say that "height has an effect on age". A statement like this does not mean that height variation causes age variation *in real life*. Instead, it means that within the context of the universe you have constructed using your model, you've framed body length as affecting age. This was done by choosing one as a dependent variable and the other as an independent variable.




The decomposition of variables as a sum of effects, and the view of effects as representing predictable (maybe not casual) variation is just a way to *think* about variables, to break up observed values into their component parts. It should not be confused with the *reality* of these values and the process that underlies them (whatever that is!). So far we've covered the fact that after picking a value to use as a reference point (the model intercept), our models:
...
### Working with the posterior samples {#c5-working-with posteriors}
...
### Manipulating the random effects {#c5-manipulating-random-effects}

Both ways of adding fixed effects presented above will also work for combining and manipulating our random effects. For example, we can get our listener random intercept using the `ranef` function (as discussed in section \@ref{c4-inspecting-random-effects}
