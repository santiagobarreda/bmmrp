...
[@@ Noah check if I avoid being too causal-y.]
...
[@@ Noah]
We want to be clear that we are not suggesting that the command above test a binary true/false hypothesis with respect to the 'real' value of the parameter in question. Instead, we are simply using using `short_hypothesis` as a useful way to calculate combinations of parameter values. 
