...
1) Easy: Analyze the (pre-fit) model that's exactly like the one in this chapter, except using the data in `exp_ex` (`bmmb::get_model("5_model_sum_coding_t_ex.RDS")`).

2) Medium: Fit a model comparing any two groups across resonance levels. 

2) Hard: Fit two models comparing any two groups across resonance levels. Compare results across models to think about group differences.
