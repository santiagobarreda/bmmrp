...
- ways to divide subject/units into factors, between and within. 
- discuss models for comparing two groups
- fit a model with two groups and discuss it
- present coding as math implementation of categorixcal diffs
- introduce treatment and sum coding
- introduce effects, not causal
- explain coefficients as effects, deviations from expectation associated with predictors
- decomposition of variation into that atributable to different effects?
- fit with sum coding and comparison of equivalence of parameters
- getting samples, manipulating samples
- combine then summarize
- using the hypothesis function
- combining parameters, recovering missing ones.
- working with and getting the random effects
...
confusion matrix, Contrasts, distributional robustness, grand mean, linearly dependent, non-standardized distribution, parameterized, robust, sum coding, t distribution, t distributions, treatment coding
...
```{r F53, fig.height = 3.5, fig.width=8, fig.cap = "(left) Distribution of apparent heights according to apparent age group. (rght) Same as left panel but presented individually for each listener. In each case, the first box of each color (the higher box) represents responses for speakers judged to be adults.", echo = FALSE}
...
We're going to re-fit the model we fit above using sum coding, and see what effect this has on the estimated coefficients.

### Description of the model

[@@ SB - check if I avoid being too causal-y.]
...
```{r F55, fig.height = 4, fig.width = 8, fig.cap = "Comparison of histogram and trace plots of posterior samples of selected parameters and combinations.", echo = FALSE}
...
[1] "(mean = 155.59, s.d. = 1.38, 95% C.I = [152.85, 158.31])"
[2] "(mean = 8.44, s.d. = 0.34, 95% C.I = [7.78, 9.12])"  
