...
[@@ Noah - check out revision below, also do we need to say something like for real values of a, b, or c or something?] [@@ NHS look sgood]

The **multivariate normal** distribution is a straightforward and very useful generalization of the normal distribution. A multivariate normal variable is a set of variables that are each normally distributed, *and* that result in another normally-distributed variable when linearly combined. Each individual variable that makes up the multivariate normal represents a **dimension** of the multivariate normal. So, if we have a three-dimensional normal distribution with dimensions $x_1, x_2$, and $x_3$, this implies that: 1) Each of $x_1, x_2$, and $x_3$ are normally distributed, and 2) If we define a new variable $x_4$ such that $x_4 = a \times x_1 +  b \times x_2 +  c \times x_3$, where $a, b,$ and $c$ can take any scalar value, then $x_4$ will be normally distributed.
...
```{r F6-6, fig.height = 3, fig.width = 8, fig.cap="(left) Average value of lpd and elpd for each model in our simulated example. (right) The same values as on the left, however, now elpd is estimated by adjusting the lpd using the number of predictors in the model, as discussed in the text.", cache = FALSE, echo = FALSE}
################################################################################
### Figure 6.7
################################################################################
...
```{r F6-9, fig.width = 8, fig.height =3, fig.cap="(left) Listener-dependent intercept effects and 95% credible intervals estimated using brms models. Crosses indicate random effects estimated by lmer. (right) Same as the left panel but showing the listener-dependent age effects.", echo=FALSE,cache=FALSE}
