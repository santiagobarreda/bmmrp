...
plot(colMeans(listener_age_differences), cex=2,
     type = 'b', col = bmmb::cols[7], pch=16, ylim = c(132,172),ylab = "Height (cm)",
     lwd=2, xlab = "Listener")
...
plot(listener_age_effects, lwd=2,lty=1,cex=2,
     type = 'b', col = darkorange, pch=16, ylim = c(-5,25),
...
The average difference between the lines in the middle plot of figure \@ref(fig:F6-2) represents the average 'main' effect for apparent age. This effect has a magnitude of 10.1 cm, representing the separation on average between the lines for apparent children and adults and the line representing the grand mean. We can see that each listener has an unpredictable difference between their lines, representing listener-dependent variation in `A1` and a meaningful effect for `A1:L` in our data. Actually, the `A1:L` interaction *entails* an `L:A1` interaction: In the process of adding an `A1:L`  interaction in your data you also add an `L:A1` interaction to your data.

Imagine the middle plot in figure \@ref(fig:F6-2) contained two flat parallel lines, separated by some amount (the effect for `A1`). In order to add a listener dependent age effect (`A1:L`) to these lines, we would need to make the separation between the lines unpredictably different for each listener. However, in doing so we *necessarily* also make the shapes of the two lines unpredictably different. This is the only way to make the separation between the lines different for each listener. However, since the shape of the line is the listener effect, by giving each line a different shape we also necessarily add an age-dependent effect for listener to our data. As a result of this, the `A1:L` interaction is *the same thing* as the `L:A1` interaction and the notational difference between them is only superficial. 
...
The previous two formulas include the marginal effects of age and listener and the interaction between them. However, they estimate all these without adaptive pooling. We know that it is advisable to estimate factors with large numbers of levels using partial pooling, regardless of how 'random' the effect may be (see section \@ref(c4-strategies)). This applies to the marginal effects of predictors such as listener, but also to the interactions between these 'random' effects and our 'fixed' effects. In order to estimate listener-dependent age effects in our model using partial pooling, we include these inside the parentheses belonging to the listener predictor, like so:

`height ~ 1 + A + (1 + A|L) + (1|S)`

The formula above tells `brms`: "Height varies as a function of an intercept and an age effect, a listener-specific intercept and age effect, and a speaker specific intercept". It also tells `brms` to estimate the speaker intercepts and age effects, and the speaker intercepts as random effects, i.e. using adaptive partial pooling. Notice the parallel structure between our models with and without random effects:

`height ~ 1 + A +  L +  A:L     +   S`

`height ~ 1 + A + (1 +   A  |L) + (1|S)`

These models have equivalent parameters, which have been horizontally aligned in the two formulas above. The `L` term in the top model is represented by the `1` in `(1 + A|L)` in the bottom model (the listener dependent intercept effect), and the `A:L` term in the top model is represented by the `A` in `(1 + A|L)` in the bottom model (the listener dependent intercept effect). The main difference between `L` and `A:L` on one hand and `(1 + A|L)` on the other, is that the latter notation tells your model to estimate these listener dependent predictors using adaptive partial pooling. When our formula has non intercept predictors, we can omit the `1` and write it as seen below:

`height ~ A + (A|L) + (1|S)`

Our model formula might specify a model as in \@ref(eq:6-1). Compared to the t distributed model we fit at the end of chapter 5, the only changes have been the addition of a new term ($A1 \colon L_{[L_{[i]}]}$) and its associated priors ($A1 \colon L_{[\bullet]} \sim \mathrm{N}(0,\sigma_{A1 \colon L}) ,\; \sigma_{A1 \colon L} \sim \mathrm{N}(0,12)$). Note that although the $A1$ predictor does not need a subscript, the $A1 \colon L_{[L_{[i]}]}$ term (the by-listener random effect for age) does. This is because although we only have a single age predictor, the interaction term estimates a different one of these for each listener, which requires the estimation of 15 such parameters in our model.
...
A1 \colon L_{[\bullet]} \sim \mathrm{N}(0,\sigma_{A1 \colon L}) \\
...
A1 \colon L_{[\bullet]} \sim \mathrm{N}(0,12) \\
...
There are many ways to measure correlation, but the most common way is by using **Pearson's correlation coefficient**, defined in equation \@ref(eq:6-2) for variables $x$ and $y$. Equation \@ref(eq:6-2) introduces a term we have not seen yet, $\sigma_{xy}$, corresponding to the *covariance* between $x$ and $y$ (often denoted by $\mathrm{cov}(x,y)$). We can see below that the correlation between two variables is the covariance divided by the product of their individual standard deviations. 
...
The variance of a variable is the expected value of squared deviations from the mean. The **covariance** of two variables is the expected value of the product of deviations of each from their respective means. The correlation can be estimating by calculating the sum of the product of deviations from the mean and dividing by the number of observations minus one, as seen in \@ref(eq:6-2).
...
Replacing $\hat{\sigma}_{xy}$ in \@ref(eq:6-2) with \@ref(eq:6-2a) yields \@ref(eq:6-3), an estimate of te correlation ($r$) between $x$ and $y$.
