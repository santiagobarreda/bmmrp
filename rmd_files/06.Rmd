...
Although interactions are invertible, conceptually it may be useful to think of them primarily one way or another. In our case, we will focus on the listener-dependence of the age effect, presented in the middle panel of figure \@ref(fig:F6-2). In the complete and total absence of an interaction between listener and age, the orange line in the figure would be horizontal at just above 10 (the marginal difference). In the presence of an interaction, our marginal and listener-dependent effects may diverge in a seemingly random manner, as seen in the figure. 
...
\mathrm{Intercept} \sim \mathrm{t}(3,156,12) \\
A1 \sim \mathrm{t}(3,0,12) \\
\sigma, \sigma_L, \sigma_{A1 \colon L}, \sigma_S \sim \mathrm{t}(3,0,12) \\
\nu \sim gamma(2, 0.1) \\ 
R \sim \mathrm{LKJCorr} (2) \\\\
\Sigma = \begin{bmatrix} \sigma_{L} & 0 \\ 0 & \sigma_{A1 \colon L} \\ \end{bmatrix} 
\times R \times
...
[@@ SB - refer to new deterministic part, say we will be omitting it in the future but mention we omit it often in text]
...
```{r F6-9, fig.width = 8, fig.height =3.5, fig.cap="(left) Listener-dependent mean effects and credible intervals estimated using brms models. Crosses indicate random effects estimated by lmer. (right) Same as the left panel but showing the listener-dependent age effects.", echo=FALSE,cache=TRUE}
...
points (lme4::ranef (lmer_model)$L[,1], pch=4,lwd=2, cex=3, col = cols)
abline (h=0,lty=3)
brmplot (random_effects[,,2], col = cols)
points (lme4::ranef (lmer_model)$L[,2], pch=4,lwd=2, cex=3, col = cols)
abline (h=0,lty=3)
```
