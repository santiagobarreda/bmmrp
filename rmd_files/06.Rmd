...
bias-variance tradeoff, correlation, covariance, cross validation, crossed, crossing, dimension, effective parameters, expected density, Holdout, in-sample, interact, interaction, interaction effects, k-fold, linear correlation, log density, main effect, multivariate normal, nesting, out-of-sample, overfit, Pearson's coefficient, penalizing, post hoc, testing data, training data, widely criterion
...
The models we fit in chapter 5 included a predictor representing the apparent age of the speaker. However, this was only a `population-level parameter` (i.e. a 'fixed' effect) meaning that it had the same value for all listeners. Another way to think about this is that our model included only the *marginal* effect for apparent age. This is the effect of apparent age on average across all listeners, *independent* of listener. This sort of effect is often referred to as a **main effect**. Someone might ask "what is the average apparent height difference between apparent children and adult females?" and you might say "about 17 cm". Which listener exactly does this statement apply to? To all of them, this is the average *overall* effect. 
...
We can consider a possible interaction between apparent adultness and listener in our data. Below we find the average height reported by each listener for apparent adults and children, and half the difference between these averages (to estimate the adultness effect). 
...
Although interactions are invertible, conceptually it may be useful to think of them primarily one way or another. In our case, we will focus on the listener-dependence of the age effect, presented in the middle panel of figure \ref(fig:F62). In the complete and total absence of an interaction between listener and age, the orange line in the figure would be horizontal at just above 10 (the marginal difference). In the presence of an interaction, our marginal and listener-dependent effects may diverge in a seemingly random manner, as seen in the figure. 

As noted several times to this point, regression models represent the difference between means and not the means themselves. In the case of interactions, interaction effects represent the difference between the marginal effect and the conditional effect. We can see values analogous to interaction effects in the right panel of figure \ref(fig:F6-2). These effects are represented by the magnitude of the arrows representing deviations from the average effect. If there were no interaction, all the arrows in the right panel of figure \ref(fig:F6-2) would have lengths of zero, or perhaps show small amounts of random variation. So, the presence of large deviations from the marginal effect, i.e. arrows with large magnitudes, suggests that our model should include coefficients that can help represent this variation between listeners. 
...
For example, consider a situation where (monolingual) listeners are divided into groups according to native language. The listener factor will be nested within first language because each speaker only has speaks one language. As a result, in such a situation a model can not estimate the listener-dependent effect for first language, i.e. the first language 'random effect' for listeners. The reason for this is very straightforward. If we do not observe the effect of first language for each level of listener, we are not in a position to say anything about the *listener-dependent* effect of first language. 

In terms of our data, this means that to estimate listener-dependent effects for apparent age we need to make sure listener is crossed with apparent age. We can see in the `listener_age_differences` above that we have average heights for both ages for all listeners, meaning that age and listener are crossed, and it is possible to estimate the interaction between these in our model.
...
However, we know that is is advisable to estimate factors with large numbers of levels using partial pooling, regardless of how 'random' the effect may be (see section \@ref{c4-strategies}). This applies to the marginal effects of predictors such as listener, but also to the interactions between these 'random' effects and our 'fixed' effects. This leads to the second way of including interactions in our model. Whenever fixed effects interact with random effects, it is most common to include these inside the parentheses belonging to the random effect they interact with, like so:

`height ~ 1 + A + (1 + A|L) + (1|S)`

When we do this, we tell our model to fit the age by listener interaction using partial pooling just like we did for the listener intercept effects. Actually, our 'listener intercept effects' are simply listener-specific deviations from the intercept, i.e. the listener by intercept interaction. When our formula has a non intercept predictor, we can omit the `1` and write it as seen below:

`height ~ A + (A|L) + (1|S)`

The formula above tells `brms`: "Height varies as a function of an intercept and an age effect, a listener-specific intercept and age effect, and a speaker specific intercept". It also tells `brms` to estimate the speaker intercepts and age effects, and the speaker intercepts as random effects, i.e. using adaptive partial pooling. 

Our model formula might specify a model as in \@ref(eq:6-1). Compared to the t distributed model we fit at the end of chapter 5, the only changes have been the addition of a new term ($A1 \colon L_{[L_{[i]}]}$) and its associated priors ($A1 \colon L_{[\bullet]} \sim \mathrm{t}(\nu, 0,\sigma_{A1 \colon L}) ,\; \sigma_{A1 \colon L} \sim \mathrm{t}(\nu,0,12)$). Note that although the $A1$ predictor does not need a subscript, the $A1 \colon L_{[L_{[i]}]}$ term (the by-listener random effect for age) does. This is because although we only have a single age predictor, the interaction term estimates a different one of these for each listener, which requires the estimation of 15 such parameters in our model.
...
\mathrm{Priors:} \\ 
L_{[\bullet]} \sim \mathrm{t}(3, 0,\sigma_L) \\
A1 \colon L_{[\bullet]} \sim \mathrm{t}(3, 0,\sigma_{A1 \colon L}) \\
S_{[\bullet]} \sim \mathrm{t}(3, 0,\sigma_S) \\
\\
\mathrm{Intercept} \sim \mathrm{t}(3, 156,12) \\
A1 \sim \mathrm{t}(3, 0,12) \\
\sigma_L, \sigma_S, \sigma_{A1 \colon L} \sim \mathrm{t}(3, 0,12) \\
3 \sim gamma(2, 0.1) \\ 
...
This model is perfectly fine, sort of. One potential problem is that we are now estimating two parameters for each listener, an intercept and an effect for `A`. Our model above draws each of these variables from independent distributions as if these were totally unrelated. The problem with treating multiple listener-specific effects as independent is that they are often not, and recognizing this fact can improve our models.
...
In figure \@ref(fig:F6-3) we plot these against the listener-dependent effects for apparent adultness (`listener_age_effects`), calculated above. In the rightmost panel we see that these two variables seem to have a negative relationship such that smaller values of average heights tend to be associated with larger apparent age effects. This is important because, if this is true, it suggests that we are unlikely to observe listeners who reported tall speakers on average *and* had large age effects on height (or vice versa). This is because as seen in figure \@ref(fig:F6-3), speakers who reported tall speakers on average tended to exhibit small age effects.

```{r F6-3, fig.height = 3, fig.width=8, fig.cap = "(left) Average reported height by each listener. (middle) The magnituve of the age effect for each listener. (right) Listener averages reported height plotted against listener age effects.", echo = FALSE}
...
Below we simulate five samples of a two-dimensional (bivariate) normal distribution by independently sampling from two univariate normal distributions and sticking these together as columns. Each row represents a single observation from the variable and each column represents a different dimension of the variable for that observation. For the variable below, the dimensions reflect the variation in the listener means and listener age effects presented in figure \@ref(fig:F6-3). So, the first dimension has a mean and standard deviation equal to the mean and standard deviation of our listener means (154 and 4.8 cm respectively), and the second dimension has a mean and standard deviation equal to the mean and standard deviation of our listener age effects (10 and 4.2 cm respectively).

```{r}
multivariate_normal = cbind(rnorm (5,154,4.8),rnorm (5,10,4.2))
multivariate_normal
```

The dimensions of the multivariate normal can have arbitrarily different means and standard deviations, but their marginal distributions will always be a (univariate) normal. However, the dimensions are not always totally independent as they are for our small sample above. In figure \@ref(fig:F6-4) we plot samples from three different bivariate normal distributions, one variable presented in each row. The variables in all three rows have the same means and standard deviations used to make our small sample above (i.e. the reflect the listener means and standard deviations). However, the rows differ in terms of the correlation between the two dimensions of the variable. 

```{r F6-4, fig.height = 6.5, fig.width = 7, fig.cap="10,000 bivatiate normal draws of simulated intercept and A1 coefficients from distributions with a means and standard deviations based on our listener data. The correlation of the variables is 0 (top), 0.5 (middle) and -0.85 (bottom). The left column presents intercepts, the right column presents A1 effects, and the right column presents both variables together.", cache = TRUE, echo = FALSE}
...
The shape of the multivariate normal distribution (i.e. how much it looks like a circle vs a tilted ellipse in the bivariate case) is determined by a covariance matrix, often represented by a capital sigma ($\Sigma$). This matrix is a square $n \times n$ matrix for a variable with $n$ dimensions. When we dealt with unidimensional normal distributions for our previous random effects, we specified priors for the (unidimensional) standard deviations using t (or normal) distributions. The specification of priors for our covariance matrix is only slightly more complicated. Rather than specify priors for $\Sigma$ directly, `brms` (and STAN) builds up $\Sigma$ from some simpler components that we specify. In this section we will provide more information than you really need to fit these models, but it helps to understand why the priors are specified the way they are for our random effects. 

Consider two random effects, a random by subject intercept $L$, and a random by-listener apparent age effect called $A1 \colon L$. The covariance matrix for our random effects is created by multiplying the standard deviations of our individual dimensions by a correlation matrix ($R$) specifying the correlations between each dimension. The operation is presented in \@ref(eq:6-4).
...
\sigma_{A1 \colon L} \sim \mathrm{t}(3,0,12) \\
...
Here is a plain English description of the model specification in \@ref{eq:6-7}:

> We are modeling apparent height as coming from a
