...
We recreate figure \@ref(fig:F5-9) as figure \@ref(fig:F6-1) below to show the distribution of apparent height by listener, and across apparent ages. In the last chapter we focused on 'the difference' between height judgments for apparent adults and apparent children as a singular thing. This is a bit like focusing only on the difference between boxes in the left panel of figure \@ref(fig:F6-1). However, if we instead focus on the right panel in figure \@ref(fig:F6-1) we see that there is not a *single* difference in heights between apparent children and adults, but rather a *set* of listener-specific differences. The model we will introduce in this chapter will be able to represent this information.

```{r F6-1, fig.height = 2.75, fig.width=8, fig.cap = "(left) Distribution of apparent heights according to apparent age group. (rght) Same as left panel but presented individually for each listener. In each case, the first box of each color (the upper box) indicates responses for speakers judged to be adults. The horizontal lines running through the figures represent the grand mean (black), the adult mean (blue), and the child mean (green).", echo = FALSE}
...
Figure \@ref(fig:F6-2) presents several ways to consider this information. In the left plot we see that the distance between the top and bottom lines, i.e. the effect for apparent age, varies from listener to listener. This listener-specific variation in the age effect is evidence of an interaction between listener and age in our data, i.e. `A1:L`. In other words, to accurately predict the effect of age (the spacing between the lines) you need to know the listener. The first plot also highlights the fact that the presence of an `A:B`interaction always indicates the presence of a `B:A` interaction: The listener-specific age effects are also age-specific listener effects. We can see the age-specific listener effects by looking at the differing shapes of the two lines in figure above, and how these vary relative to their respective group means. The shapes are different, indicating that the mean for each listener varies conditionally based on apparent age. This conditional effect reflects an interaction between listener and apparent age in our data, i.e. `L:A1`.
...
In fact, the `A1:L` (the effect of apparent age dependent on listener) encodes the same information as `L:A1` (the effect of listener dependent on apparent age). Imagine the left plot in figure \@ref(fig:F6-2) contained two flat parallel lines, separated by some amount. In order to add a listener dependent age effect (`A1:L`) to these lines, we would need to make the separation between the lines unpredictably different for each listener. However, in doing so we *necessarily* also make the shapes of the two lines unpredictably different. In fact, it is the arbitrary difference in shapes between the two lines (`L:A1`) on the left of figure \@ref(fig:F6-2) that allows there to be a listener dependent age effect (`A1:L`). As a result of this, the `A1:L` interaction is *the same thing* as the `L:A1` interaction and the notational difference between them is only superficial. 

Although we can think of interactions either way, conceptually it may be useful to think of them primarily one way or another. In our case, we will focus on the listener-dependent effect for apparent age, presented in the middle panel of figure \@ref(fig:F6-2). The value of each point along the line in the middle plot reflects the spacing of the two lines in the left plot of the same figure. In the complete and total absence of an interaction between listener and age, the orange line in the figure would be horizontal at just above 10 (the marginal difference). In the presence of an interaction, our marginal and listener-dependent effects may diverge in a seemingly random manner, as seen in the figure. 

As noted several times to this point, regression models represent the difference between means and not the means themselves. In the case of interactions, interaction effects represent the difference between the marginal effect and the conditional effect. We can see values analogous to interaction effects in the right panel of figure \@ref(fig:F6-2). These effects are represented by the magnitude of the arrows, which reflect listener-specific deviations from the average effect for apparent age. If there were no interaction, all the arrows in the right panel of figure \@ref(fig:F6-2) would have lengths of zero, or perhaps show small amounts of random variation. So, the presence of large deviations from the marginal effect, i.e. arrows with large magnitudes, suggests that our model should include coefficients that can help represent this variation between listeners. 
