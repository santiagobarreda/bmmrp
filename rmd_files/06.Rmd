...
Our model formula might specify a model as in \@ref(eq:6-1). Compared to the t distributed model we fit at the end of chapter 5, the only changes have been the addition of a new term ($A \colon L_{[L_{[i]}]}$) and its associated priors ($A \colon L_{[\bullet]} \sim \mathrm{N}(0,\sigma_{A1 \colon L}) ,\; \sigma_{A \colon L} \sim \mathrm{N}(0,12)$). Note that although the $A1$ predictor does not need a subscript, the $A1 \colon L_{[L_{[i]}]}$ term (the by-listener random effect for age) does. This is because although we only have a single age predictor, the interaction term estimates a different one of these for each listener, which requires the estimation of 15 such parameters in our model.
