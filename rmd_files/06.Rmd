...
- multivariate normal, random effects drawn from it
- fitting and interpreting random effect
- model comparison, overfitting, bias variance trade off
- in sample and out of sample prediction
- pensalizing likelihoods
- distinguishing in and out of sample data
- 
- estimating out of sample prediction using in sample
...
> We are modeling apparent height as coming from a t distribution with unknown nu, mean, and scale parameters. The expectec value for any given trial is modeled as the sum of an intercept, an effect for apparent age, a listener effect, a listener dependent effect for apparent age, and a speaker effect. The speaker effects were drawn from a univariate normal distribution with a standard deviation estimated from the data. The two listener effects were drawn from a bivariate normal distribution with standard deviations and a correlation that was estimated from the data. The reaminder of the 'fixed' effects and the bivariate correlation were given fixed priors. 
...
This section of the model summary now includes an estimate of the standard deviation of the by-listener effect for age (i.e. the standard deviation of the listener by age interaction, $\sigma_{A1 \colon L}$ in \@ref(eq:6-7)), and the correlation between our listener-dependent intercept and age-effect terms. We can see that the estimate of this correlation (-0.80) is very similar to the correlation estimated for these terms in our example above (-0.85). 
...
The standard deviation of our listener intercepts appears to decrease slightly at the inclusion of random effects for age. This is likely because what previously seemed like variation in listener averages may have actually been variation in listener age effects. The separation of this into two separate predictors may lead to the magnitude of variation in one being diminished. Finally, the decrease in the error standard deviation is due to the inclusion of the listener random effects for `A1`, and the fact that including listener-dependent values for `A1` helped explain our variable. The error is just what the model can't explain, so as our models explain more, the error will tend to get smaller. 
...
And \@ref(eq:6-11), where the $\tilde{y}$ is the out-of-sample data being predicted using the in-sample estimates of $\mu$ and $\sigma$.
...
```{r F6-6, fig.height = 3, fig.width = 8, fig.cap="(left) Average value of lpd and elpd for each model in the simulations above. (right) The same values as on the left, however, now elpd is estimated by adjusting the lpd using the number of predictors in the model.", cache = TRUE, echo = FALSE}
################################################################################
### Figure 6.6
################################################################################
...
text (1:3, means[1:3]-.25-c(2:4), labels = means[1:3]-c(2:4))
...
```{r F68, fig.height = 3, fig.width = 8, fig.cap="(left) Standardized model residuals plotted against values of the WAIC penalty term (P_WAIC) for each data point for the model with Gaussian errors, `model_sum_coding`. (right) The same as on the left but for the model with t distributed errors, `model_sum_coding_t`.", cache = TRUE, echo = FALSE}
...
We can see that the `elpd_diff` is just the sum of the `elpd` estimates across the two models:

```{r}
sum(model_t_loo_info[,1]-model_re_t_lo_info[,1])
```

The standard error of the difference is simply the standard deviation of the difference times the square root of N. 
...
We can see that the more-complex model in this case actually has a slightly higher $\mathrm{elpd}$. In addition, we can look at a summary of the fixed effects and see that the value of the useless predictor is near zero (0.08), and the credible interval is wide and approximately symmetrical about zero. So, does this mean that this predictor should be excluded from the model? We would be justified in doing so, but may not necessarily want to. For example, if one of our important research questions centered round the value of the useless predictor, the fact that it is around zero and not some other value (e.g. -100) may be useful information. For example, we might want to keep the predictor in the model so we can say something like "So & So (2007) reported a value of -100 for the useless predictor. However, our results suggest that the effect of this predictor are likely to be zero or at least quite small [mean = 0.08, sd = 0.20, 95% CI = [-0.31,0.48])". 

We can see that the more-complex model has a slightly higher $\mathrm{elpd}$, but that this difference in only 1/4 the magnitude of the standard error. In addition, we can look at a summary of the fixed effects and see that the value of the useless predictor is near zero (0.3), and the 95% credible interval is wide relative to this value and include zero ([-.2, .8]). 

```{r, cache = TRUE}
brms::fixef(model_re_t_tooBig)
```

Does this mean that this predictor should be excluded from the model? We would be justified in doing so, but may not necessarily want to. For example, if one of our important research questions centered round the value of the useless predictor, the fact that it is around zero and not some other value (e.g. -100) may be useful information. We might want to keep the predictor in the model so we can say something like "So & So (2007) reported a value of -100 for the useless predictor. However, our results suggest that the effect of this predictor are likely to be near zero [mean = 0.34, sd = 0.25, 95% CI = [-0.17,0.84])". 
...
Our answers to the main research questions are largely unchanged from the previous chapter (section \@ref{c5-answering-qs}), however, there have been some changes that highlight interesting aspects of our data. We are going to focus on what our model tells us the listener dependent intercepts and effects for apparent age. 
