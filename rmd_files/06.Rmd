...
In this section we will offer a high-level conceptual understanding of Bayesian model comparison, along with an explanation of how to carry this out. For more information on the subject we recommend reading chapter 7 of the truly excellent *Statistical Rethinking* (McElreath, 2020). For an even more thorough treatment on the subject please see X and X. Much of the information provided in this section is provided in more detail in Vehtari et al. (2017), and with more mathematical detail.
...
Which model should we use? Based on the comparison above, we see that `model_re_t` has an $\mathrm{elpd}$ that is 204 greater than the next best model. How large does an $\mathrm{elpd}$ difference need to be in order to be meaningful? There two things that need to be taken into account: The magnitude of a difference and the uncertainty in the difference. A general rule of thumb seems to be that a difference of around 4 in $\mathrm{elpd}$ suggests a meaningful difference between the models (Vehtari et al. 2017). However, this is simply a rule of thumb and does not mean that a difference of 3.99 does not matter and a difference of 2.01 does. On the other hand, even large differences between models may not be reliable in the presence of large standard error estimates. For example, below we see the difference between out two smaller models:
