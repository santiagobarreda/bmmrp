...
```{r F6-4, fig.height = 6.5, fig.width = 7, fig.cap="10,000 bivatiate normal draws of simulated intercept and A1 coefficients from distributions with a means and standard deviations based on our listener data. The correlation of the variables is 0 (top), 0.5 (middle) and -0.85 (bottom). The left column presents intercepts, the right column presents A1 effects, and the right column presents both variables together.", cache = FALSE, echo = FALSE}
...
```{r F6-6, fig.height = 3, fig.width = 8, fig.cap="(left) Average value of lpd and elpd for each model in the simulations above. (right) The same values as on the left, however, now elpd is estimated by adjusting the lpd using the number of predictors in the model.", cache = FALSE, echo = FALSE}
################################################################################
### Figure 6.6
################################################################################
...
```{r F6-9, fig.width = 8, fig.height =3.5, fig.cap="(left) Listener-dependent mean effects and credible intervals estimated using brms models. Crosses indicate random effects estimated by lmer. (right) Same as the left panel but showing the listener-dependent age effects.", echo=FALSE,cache=FALSE}
