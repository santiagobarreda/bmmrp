...
Figure \ref(fig:F62) presents several ways to consider this information. In the left panel we see that the distance between the top and bottom lines, reflecting the effect for apparent age, varies from listener to listener. So, the effect of apparent age is dependent on the level of listener you are observing. Recall from chapter 2 that if the conditional distribution of a variable changes as a function of another variable then these are variables are not independent. In the same way, if the conditional effect of a predictor changes as a function of another predictor, then these effects are not independent. In the middle panel we see how the listener-specific age effect varies around the average (marginal, or main) effect, indicated by the horizontal line.



Recall from chapter 2 that variables are said to be independent when their marginal distributions match their conditional distributions. The same can be said of effects: Effects are independent when the marginal effects are equal to the conditional effects. In terms of the middle panel in figure \ref(fig:F62), this means that we would like every point on the line, the listener-dependent effects, to fall exactly on the horizontal line. When two variables do not have independent effects on your outcome, they are said to **interact** and have an **interaction**. In the right panel we see what can be thought of as interaction effects, the difference between the main (average) effect and the listener-dependent effect. If there is no interaction, then the effect of adultness conditional on listener is equal to the main effect for adultness. In other words, in the absence of a listener by adultness interaction, all the arrows in the right panel of figure \ref(fig:F62) would have lengths of zero, or perhaps show small amounts of random variation.   



The parameters in your model that help you capture these conditional effects are referred to as interactions or **interaction effects**. So, the 'fixed' effect for `A1` really represents the marginal (overall) effect for the predictor. To investigate the values of this predictor *conditional* on listener, we need to include the listener by `A1` *interaction* in our model.
