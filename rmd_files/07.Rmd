...
Q1) Is average apparent speaker height different based on apparent speaker category?

[@@ NHS: I find that phrases like "based on" kind of bother me here - we don't really know if people are categorizing the stimuli as b/g/m/w first and then basing their height judgments (in part) on the category, right? maybe they do both simultaneously, or maybe they judge height and that informs the category...]

[@@ SB - I went back over the earlier chapters to remove causal stuff like this]
...
[@@ NHS: I don't understand what `x1` is here or why `x2` and `x3` aren't in the formula...]
[@@ SB - this was basically a typo]
...
We can use `brmplot` from the `bmmb` package to easily make plots showing means and credible intervals for our fixed effects, including the recovered coefficient. We also plot the two comparisons we made using the `hypothesis` function above, presented in figure \@ref(fig:F7-3).
  The distributions on the left tell us about probable values for our category parameters: Given our data and model structure, there is a 95% probability that the parameter value falls within this interval [@@ NHS: shouldn't this be (something like) "this is the most probable 95% set of possible parameter values"? the way it's written sounds like a very frequentist, CI-focused interpretation (and incorrect with respect to what CIs are/mean)]. Our category effects reflect the expected difference between our model intercept, the overall grand mean, and each category mean. So, the intervals on the left of figure \@ref(fig:F7-3) tell us that there is a 95% probability (given our data and model structure) that the actual difference between the grand mean and each group mean falls within that range [@@ NHS: ditto]. What do the distributions in the right plot in figure \@ref(fig:F7-2) tell us? They represent the posterior distribution of the difference between category means directly, without involving the intercept. 
...
[@@ NHS: left panel is mislabeled, I think - m and w have lower height estimates (and more variability) than b and g.]
...
[@@ NHS: should the deterministic equation relating $\Sigma$ to $R$ and the various $\sigma$ parameters be given here, too?]
...
The value of $\mu$ above is the expected value for a given trial. This is also sometimes called the **linear predictor** [@@ NHS: I believe you that it's called this sometimes, but this phrase doesn't feel familiar to me. I might also add in "conditional expectation function", which is a useful descriptor from econometrics]. Since the linear predictor is the value you expect for a trial, it's obvious that there should be a reasonable correspondence between the linear predictor and your dependent variable. If what your model tells you to expect is not like what you actually observe, then what good is the model? One shortcoming of the linear predictor is that this does not incorporate error[@@ NHS: error or uncertainty? Or both?]. As a result, the linear predictor is constant for any single combination of dependent variables, and undersells the amount of variation that can be expected in a given data set. The **posterior predictive distribution** is the distribution of possible data given your parameter estimates and probability model. In other words, the posterior predictive distribution is your linear predictor, plus the random error [@@ NHS uncertainty?] that you expect in your model. For normally distributed data, our posterior predictive distribution is the distribution of generated values of $\tilde{y}$ given estimates of $\mu$ and $\sigma$ as in \@ref(eq:77).

[@@ NHS: my notes about uncertainty are meant to try to clarify the distinction between error, which I feel like we are typically using to refer to the residuals for a given model, and uncertainty, i.e., the variation in parameter estimates and the "error" in the partially pooled model components]
...
Below we see the data frame corresponding to our posterior predictions. Note that although the means of the linear and posterior predictions are very similar, the intervals around the posterior predictions are noticeably wider. This is because they incorporate error [@@ NHS: same note as above - error and/or uncertainty? I think it's important to be consistent with our terms]. Another difference between the linear and posterior predictions is that the linear predictors are fixed for a given set of posterior samples. In contrast, every time you run the command `predict(model)` you will get slightly different results because these incorporate random error. 
...
[@@ NHS: a high-level comment on this chapter: a little bit more motivation and justification for cranking through some of the calculations would be useful for the reader, e.g., when comparing the different groups means in the data vs what the model w/o interactions predicts, making it clear that this is why the calculations are being carried out up front would be useful. That is, having a few sentences saying that it can be useful to calculate data-based statistic and compare them to model predictions would be useful for the reader to know why we turn from a fitted model directly to calculating otherwise non-obvious quantities]
