...
The model in this section will use the following variables from our data frame: 
...
boxplot (height ~ C, data = exp_data, col = cols[1:4],ylim=c(100,200), 
         xlab = "Apparent Category", ylab = "Height (cm)")
boxplot (height ~ C_v, data = exp_data, col = cols[1:4],ylim=c(100,200),  
...
We can use the `brmplot` in the `bmmb` package to easily make plots showing means and credible intervals for our fixed effects, including the recovered coefficient. We also plot the two comparisons we made using the `hypothesis` function above. The distributions on the left tell us about probable values for our parameters: Given our data and model structure, there is a 95% probability that the parameter value falls within this interval. Our category effects reflect the expected difference between our model intercept, the overall grand mean, and each category mean. So, the intervals on the left of figure \@ref(fig:F7-2) tell us that there is a 95% probability (given our data and model structure) that the actual difference between the grand mean and each group mean falls within that range. What do the distributions in the right plot in figure \@ref(fig:F7-2) tell us? They represent the posterior distribution of the difference between category means directly, without involving the intercept. Conceptually, finding the difference between the intercept and a group mean is no different than finding the difference between individual group means. Consider the example of providing directions with respect to your house. If the library is 5 miles west and the park is 3 miles west, you could refer to these as -5 and -3 miles from your house, west to east. However, you could also express the difference between one and the other directly as 2 (or -2). The left plot in figure \@ref(fig:F7-2) expresses all distances with respect to the intercept (your house) while the right plot expresses differences with respect to individual category means directly (the library and park). 

```{r F7-3, fig.height = 3, fig.width=8, fig.cap = "(left) Means and 95% credible intervals for group effects for our four group model. H1 through H4 reflect the effects for boys, girls, men, and women respectively.  (right) bla.", echo = FALSE}
...
Our model contains two predictors with two levels each, leading to four groups. Height judgments for these four groups are presented in figure \@ref(fig:F7-4). These four boxes correspond to the four cells in figure \@ref(fig:F7-3). The middle plot in \@ref(fig:F7-4) compares height judgments across levels of gender (male vs. female). Since we have both adults and children within each gender group, each box in the plot is really the average of two of the boxes from the left plot in \@ref(fig:F7-4) (boxes 1 and 3 vs. 2 and 4). This is equivalent to finding the average of one row in \@ref(fig:F7-3), and comparing it to the average of the other row. Similarly, the right plot in \@ref(fig:F7-3) compares height judgments across levels of age (adult vs. child). Since we have both males and females within each gender group, each box in the plot is really the average of two of the boxes from the left plot in \@ref(fig:F7-4) (boxes 1 and 2 vs. 3 and 4). This is equivalent to finding the average of one column in \@ref(fig:F7-3), and comparing it to the average of the other column. In this way, our model including two fixed effect predictors can use four groups to answer two questions simultaneously: What is the effect of apparent age on apparent height? What is the effect of apparent gender on apparent height? 
...
par (mfrow = c(1,3))
boxplot (height ~ G+A, data=exp_data, main = "Overall", ylim = c(90,200),col=cols[3:6])
...
plot (0:1,tapply (exp_data$height,exp_data$G,mean), col=3, ylim = c(130,190),xaxt='n',
      lwd=3,type='b',pch=16,cex=1.5,main="Gender",xlim=c(-.2,1.2),ylab='f0',xlab='')
axis (at=0:1, labels = c('female','male'), side=1)

G = exp_data$G
A = exp_data$A
bmmb::interactionplot (G, A, exp_data$height,col=3:4, lwd = 3, leg.y = 170, 
                  leg.x=1.8,type='b',pch=c(16,17),cex=1.5, ylim = c(130,190),
                  main="Both",ylab='f0', xlim = c(.9,2.5))

plot (0:1,tapply (exp_data$height,exp_data$A,mean),col=3, ylim=c(130,190),xaxt='n',lwd=3,
      type='b',pch=16,cex=1.5, main = "Adult",xlim=c(-.2,1.2),ylab='f0',xlab='')
axis (at=0:1, labels = c('adult','child'),side=1)

bmmb::interactionplot (A, G, exp_data$height,col=3:4, lwd = 3, leg.y = 170, 
                  leg.x=1.8,type='b',pch=c(16,17),cex=1.5, ylim = c(130,190),
                  main="Both",ylab='f0', xlim = c(.9,2.5))
```
...
interactionplot (exp_data$G, exp_data$A, exp_data$height, col=3:4, ylim=c(140,180),lwd=3,
                  type='b',pch=c(16,17),cex=1.5,main="Data",legend = FALSE)

interactionplot (exp_data$G, exp_data$A, y_post_pred_no_re[,1],col=3:4, lwd=3,
                  type = 'b', pch = c(16,17), cex = 1.5, ylim = c(140,180),
                  main="Pred. without RE", leg.x=1.8,leg.y=270, xlim=c(.95,2.3))

interactionplot (exp_data$G, exp_data$A, y_post_pred[,1], col = 3:4,lwd=3, type='b',
...
Before continuing, we want to address the fact that we've included the same structure in our 'fixed effects' as for our listener 'random effects'. We will do this for all the models we fit in this book, and there are a couple of reasons for this. First, not including random effects for a predictor (e.g., apparent age) is assuming that that variance component is equal to zero (i.e. $\sigma_A=0$), which may not be the case. Second, not including a 'random' effect for a parameter according to a grouping factor effects adopts a 'complete pooling' approach to the estimation of the parameter across levels of the grouping factor. Both of these positions as *assumptions* that may or may not be true about our data, and can cause problems when they are wrong. 
  In chapter 6 we saw that there is substantial variation in the listener-dependent effects for apparent age. We fit a model with and without random effects and saw that the model with random effects produced a substantially wider credible intervals for the apparent age 'fixed' effect, reflecting increased uncertainty in some parameter estimates. In that case, assuming listener-variation in the parameter was zero when it was not resulted in an overly precise estimate of that parameter. The only way to know whether a listener-dependent parameter shows substantial variation or not is to include the possibility for listener-dependent variation in your model, at least initially. Of course, the inclusion of any 'random effect' in your model assumes that: 1) Your model is designed in a way that the 'random' effect can be estimated, and 2) You have enough data to estimate those parameters. These issues will be discussed further in chapter 11.   
...
model_interaction =  
  brms::brm (height ~ A + G + A:G + (A + G + A:G|L) + (1|S), data = exp_data, chains = 4, cores = 4,
       warmup = 1000, iter = 5000, thin = 4, prior = priors, family = "student")
```
```{r, include = TRUE, eval = FALSE}
# Or download it from the GitHub page:
...
interactionplot (exp_data$G, exp_data$A, exp_data$height, col=3:4, ylim=c(140,180),lwd=3,xlab="",
...
interactionplot (exp_data$G, exp_data$A, y_post_pred[,1],col=3:4, lwd=3,
                  type = 'b', pch = c(16,17), cex = 1.5, ylim = c(140,180),xlab="",
                  main="No Interaction", legend = FALSE, ylab = 'Apparent Height (cm)')
segments (1,ps[1],2,ps[2], col = coral,lty=3,lwd=4)
segments (1,ps[3],2,ps[4], col = coral,lty=3,lwd=4)

interactionplot (exp_data$G, exp_data$A, y_post_pred_no_re_int[,1],col=3:4, lwd=3,
                  type = 'b', pch = c(16,17), cex = 1.5, ylim = c(140,180),xlab="",
                  main="Interaction", legend = FALSE, ylab = 'Apparent Height (cm)')
segments (1,ps[1],2,ps[2], col = coral,lty=3,lwd=4)
segments (1,ps[3],2,ps[4], col = coral,lty=3,lwd=4)

interactionplot (exp_data$G, exp_data$A, y_post_pred_int[,1],col=3:4, lwd=3,
                  type = 'b', pch = c(16,17), cex = 1.5, ylim = c(140,180),xlab="",
                  main="Interaction", legend = FALSE, ylab = 'Apparent Height (cm)')
segments (1,ps[1],2,ps[2], col = coral,lty=3,lwd=4)
segments (1,ps[3],2,ps[4], col = coral,lty=3,lwd=4)
...
We can inspect the output and see that they have very similar values. This is not surprising given their similarities seen in figure \@ref(fig:F7-9). 
...
Figure \@ref(fig:F7-10) compares our $R^2$ values with and without random effects. These values just formalize what we saw in our interaction plots in figure \@ref(fig:F7-9), that the interaction term helps model fit in a noticeable way if we only consider the model fixed effects structure. 

```{r  F7-11, fig.height=3,fig.width=8,fig.cap=".", echo = FALSE}
...
par (mfrow = c(1,2), mar = c(4,4,1,1))
bmmb::brmplot(rbind(r2_both,r2_interaction), ylim = c(.755,.80),
...
```{r F7-12, fig.height=3,fig.width=8,fig.cap=".", echo = FALSE}
...
Figure \@ref(fig:F7-11) also presents the pairwise difference between all four apparent speaker groups (boys, girls, women, and men). The difference between group means is calculated by extracting the samples from the group means we reconstructed in section 7.X (`means_pred_interaction`). The result of this is a matrix with four columns, each representing a group effect, with individual posterior samples differing along rows. To find the difference between group means between each pairs of groups, we subtract the columns representing the group effects and inspect the distribution of the differences. This process is shown below and the distribution of pairwise group differences in presented in figure \@ref(fig:F7-11).
