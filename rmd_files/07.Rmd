...
we've been making the details of our models more complicated, but we have still only fit models with extremely simple fixed effects structure. In this chapter we are going to simplify many of the other modeling details we have introduced so far, and introduce more complexity into our fixed effects model structure instead. 
...
```{r F4-designfig, echo = FALSE, out.width = "80%", fig.cap = "Data from a single group of subjects (S) divided according to a single grouping factor A.", fig.align = "center"}
...
```{r F4-datacomparison, echo=FALSE,fig.height = 3, fig.width = 8, fig.cap = "(left) Comparison of the four groups (middle) Comparison of productions based on whether the speaker is an adult (right) Comparison of all productions based on whether the speaker is male."}
...
```{r F4-designfig2, echo = FALSE, out.width = "100%", fig.cap = "Data from four groups of subjects (S) divided according to a two grouping factors A and B.", fig.align = "center"}
...
## Investigating model fit with posterior prediction
...
we previously introduced interactions as conditional effects. these were in the context of our random effects, group specific adjustments to our parameters. now we are going to use the idea of conditional effects to understand our fixed effects predictors. 
...
```{r F4-interactionplot, fig.height=3,fig.width=8,fig.cap="Plots showing different ways to consider our f0 data.", echo = FALSE}
...
### Interactions in our height data
...
```{r F4-interactionfig, echo = FALSE, out.width = "100%", fig.cap = "An interaction plot built up from its individual components."}
