...
Abstract:
This chapter introduces prior predictive checks, and talks about the importance of these for model building. Then, examples of the results of different prior settings for models are provided. After that the chapter discusses how to specify more specific prior probabilities for individual model parameters. The chapter then introduces heteroscedastic models, that is, models with error terms that vary from observation to observation. First, the chapter presents a ‘simple’ model that includes only variation in the error term across two conditions. Following that, it includes a ‘complex’ model that features listener-dependent error terms fit using shrinkage, in addition to the equivalent of listener ‘random effects’ for the error term. Finally, the chapter discusses building identifiable models, and models supported by the available data, and describes the problems of collinearity, linear dependance, and saturated models.  
...
To this point our models have all looked something like \@ref(eq:8-1), with the generic predictors $x_p$. In these models $\mu$ got a subscript because it varied from trial to trial and was being predicted, but $\sigma$ has never received a subscript because it hasn't varied in our models.  
...
Instead, we can fit models that help us understand systematic variation in both the mean *and* the $\sigma$ parameters in our data. We can imagine this by adding a subscript to $\sigma$ and predicting its value from trial to trial using variables, as seen in \@ref(eq:8-2). We've given these predictor variables little $\sigma$ subscripts just to distinguish them conceptually from those predicting $\mu$, but they could be the same variables used to understand $\mu$ or a different set. 
...
1) Easy: Analyze the (pre-fit) model that's exactly like the one in this chapter, except using the data in `exp_ex` (`bmmb::get_model("8_model_A_L_sigma_ex_ex.RDS")`).
