...
[@@ NHS: we should maybe (probably) discuss model identifiability here, too. The number of observations vs the number of parameters is often relevant and the only thing you need to worry about, but it's not always so simple to just count up observations and model parameters. In some models, parameter can trade off against each other (I think we've probably already discussed some examples of this with the categorical variable stuff and the fact that you can't estimate coefficients for all of the levels of a factor). I suppose it's a bit of a cop-out for me to say we should do this but not just go ahead and do it, but, well, I'm kind of hung over today, so this is what I can currently muster, energy and focus wise.]

[@@ SB - I might have to add a tiny section at the end of this chapter about this or something. correlation between predictors too]
...
[@@ NHS: looks like you kind of ran out of steam here...]

- taking a break from comparison. 
- going to simulate multivar random effects.
- simulate different error variances.
- we will simulate some of our differences

## Simulating data using our model parameters {#c8-simulating}
...
WBelow we make two data sets that are 'incomplete' with respect to the variation in our data. The first contains the intercept and noise only, the second contains the intercept and listener effects only.
