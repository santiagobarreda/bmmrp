...
The models in this chapter seek to estimate five parameters for each listener ($Intercept, A1, G2, A1:G1, \sigma$) based on 139 observations for each listener. This is a reasonable ratio of observations to parameters. However, if we had only 10 observations per listener, it might not be such a good idea to estimate so many parameters for each listener. In general, before fitting a model that you *can* fit, it's a good idea to think whether you *should* fit it given the nature of your data and the number of observations you have overall (and in different conditions).

Imagine we wanted to fit a model with speaker effects, listener effects, *and* the interaction between the two. This is effectively asking to what extent listeners exhibit different responses for individual listeners. This model would need to represent the 15 listener effects, and also 139 speaker effects conditional on each listener. This means that at a minimum, this model would require 15 x 139 = 2085 parameters to represent these effects. However, we also only have 2085 data points, one observation per speaker per listener. As a result, we would have a different parameter for every single observation leading to what's known as a **saturated model**. 

In a model without shrinkage of parameters, saturated models leads to perfect prediction of data. This means that there is no data-level error ($\sigma$), or in fact, any random variation at all. This is problematic because the existence of a non-zero $\sigma$ is a pre-requisite for the calculation of the likelihood function of our regression parameters (see section X, chapter 2). For example, the model below doesn't report errors because it doesn't fit at all, it repeatedly crashed R and Stan when we attempted to fit it.
...
What if we had not one but two observations per speaker per listener? Now this would not be a saturated model, but we could think of it as *nearly-saturated* a term that may not be 'real' but gets the point across. A saturated model fails because once you estimate all your parameters, there is not enough uncertainty left over for it (the uncertainty) to be effectively estimated.  
