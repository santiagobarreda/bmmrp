...
We're going to focus on models whose structure is similar to the final model we fit in chapter 7 (`model_interaction`). However, in this chapter we are going to focus on questions related to variation in our parameters. We would like to know three things:
...
```{r F83, fig.height = 5, fig.width = 8, fig.cap="--", cache = TRUE, echo = FALSE}
################################################################################
### Figure 8.3
################################################################################
...
In this section we will fit a distributional model to our data, we will seek to understand systematic variation in both the mean and standard deviation parameters in our model. This will be done by, effectively, adding a subscript to $\sigma$ and predicting its value from trial to trial using predictors, as seen in \@ref(eq:8aaa). We've given these predictors little $\sigma$ subscripts just to distinguish them conceptually, but they could be the same predictors used to understand $\mu$ or a different set of predictors. 
...
#  saveRDS (prior_A_sigma,"../models/8_prior_A_sigma.RDS")
...
Since our `sigma` parameters are bounded by zero, our model actually estimates `log(sigma)` and not `sigma` directly. This means that to get our values of sigma we need to exponentiate our parameter estimates, which we can do directly in the `hypothesis` function as seen above. The three estimated error terms are presented in figure \@ref(fig:F8X) below. Clearly, there are substantial differences in the value of `sigma` based on perceived adultness of the speaker, the value of sigma was 61% greater (7.1/4.4) when listeners identified the speaker as a child. 

```{r F84, fig.height = 3, fig.width = 8, fig.cap="--", cache = TRUE, echo = FALSE}
...
brmplot (sigmas, col = cols[4:6])
brmplot (fixef(model_A_sigma)[c(3:5),], col = cols,nudge= -.1,pch=15)
brmplot (fixef(model_interaction)[2:4,], col = cols, add = TRUE,nudge=.1,labels="")
#abline (h = 7.71)
```
...
This told `brms` "model sigma as a function of an intercept and an effect for apparent age". We can add to our formula by specifying a 'random effects' term for listeners, and a 'random slope' for age (i.e. an interaction between apparent age and listener, estimated using partial pooling).

`sigma ~ A + (A|L)`

This formula tells `brms` "model sigma as varying around an overall intercept, an effect for apparent age, listener-specific deviations, and the interaction of apparent age and listener". We can 'stick' our two model formulas together using the `bf` (Bayes formula) function as shown below:
...
\begin{bmatrix} L_{[\bullet]} \\ A1 \colon L_{[\bullet]} \\ G1 \colon L_{[\bullet]} \\ A1 \colon G1 \colon L_{[\bullet]} \\ L_{\sigma[\bullet]} \\ A1 \colon L_{\sigma[\bullet]} \end{bmatrix} \sim \mathrm{MVNormal} \left( \begin{bmatrix} 0 \\ 0 \\ 0 \\ 0 \\ 0 \\ 0 \\ \end{bmatrix}, \Sigma \right) \\ 
\\
...
There is not much new here, other than that we are modelling parameters related to sigma ($L_{\sigma}, A1 \colon L_{\sigma}$) as part of a multivariate normal variable, just as we did for our other listener 'random effects'. Our course, this also means that we will get estimates of the standard deviations of $L_{\sigma}$ and $A1 \colon L_{\sigma}$, and in the correlation of these to the other listener-dependent parameters.
...
If we look at the model output we see see the same `sigma_Intercept` and `sigma_A1` we saw before. There are also a few new lines in the listener `Group-Level Effects`: `sd(sigma_Intercept)` and `sd(sigma_A1)`, which represent the listener-specific intercepts (for `sigma`) and age effects (for `sigma`). 

```{r, eval = FALSE}
bmmb::short_summary (model_A_L_sigma)
```

Our listener-specific adjustments to the sigma intercept were fit with partial pooling, i.e. they are 'random' effects. As such, we can get these in the same two ways indicated for other random effects terms in section X. Here, we will use the `hypothesis` function to get the listener random effects themselves (`sigmas_centered`), and the sum of these with the sigma intercept (`sigmas`). 
...
We compare these in the plot below,  exponentiating the recreated `sigma` values to get the listener-specific $\sigma$ terms. Just to be totally clear on what these are. The left plot in the figure shows us the values of $L_{\sigma}$, the listener-dependent variations from the `sigma` intercept, expressed as a logarithm. The right plot on the figure shows the $\sigma$ estimated individually for each listener. This is the result of $\exp(\mathrm{Intercept}_{\sigma} + L_{\sigma[i]})$ for listener $i$. We exponentiate the value in the plot so it reflects `sigma` rather than `log(sigma)`.

```{r F85, fig.height = 3, fig.width = 8, fig.cap="--", cache = TRUE, echo = FALSE}
...
It certainly seems as though there is legitimate between-listener variation in the size of $sigma$. We might wonder, is this added complexity a good thing? If we are mostly worried about the ability of our model to tell us something about the data we have not seen, we should worry about the $\mathrm{elpd}$. We again ass the `loo` criterion to our model:
...
We haven't talked about the speaker random effects very much as these have not been directly involved in our research questions. The speaker effects reflect systematic speaker-dependent variation that is not captured by any of the other predictors in the model. These are *supposed* to be normally distributed with a fixed standard deviation for all our speakers. Below, we check out the speaker-dependent intercept effects from `model_interaction`. 

```{r F86, fig.height = 3.5, fig.width = 8, fig.cap="--", cache = TRUE, echo = FALSE}

################################################################################
### Figure 8.6
################################################################################

par (mfrow = c(2,1), mar = c(0.5,4,0.5,1))
brmplot (ranef(model_interaction)$S[,,1],ylim=c(-11,8), line=FALSE,
         col = rep(cols[2:5], times =c(27,20,54,47)),labels = '')
abline (h=0,col="grey40")
brmplot (ranef(model_A_L_sigma)$S[,,1], ylim=c(-11,8), line=FALSE,
         col = rep(cols[2:5], times =c(27,20,54,47)),labels='')
abline (h=0,col="grey40")
```

We can see that it appears that boys, girls, men, and women have different degrees of separation around zero. We can easily build a model that includes this variation using `brms`. 

### Description of our model

Our model will be just like the `model_interaction`, which looked liked this:

`height ~ A + G + A:G + (A + G + A:G|L) + (1|S)`

This model has a single value of $\sigma_S$ for all speakers. We want our model to represent a different standard deviation for each speaker category, e.g. $\sigma_{S_b}$ for boys and $\sigma_{S_w}$ for women. To do this we use the `gr` function to specify the speaker random effect. We specify that we want to group `S` by the levels of `C_v` by replacing `(1|S)` with `(1|gr(S, by=C_v))` in our formula:

`height ~ A + G + A:G + (A + G + A:G|L) + (1|gr(S, by=C_v))`

The above formula specifies a model such as below. This model is exactly like the one for `model_interaction`, save for the separation of $\sigma_{S}$ into four parameters.
...
\mathrm{Priors:} \\ 
S_{b[\bullet]} \sim t(3,0,\sigma_{S_b}) \\
S_{g[\bullet]} \sim t(3,0,\sigma_{S_g}) \\
S_{m[\bullet]} \sim t(3,0,\sigma_{S_m}) \\
S_{w[\bullet]} \sim t(3,0,\sigma_{S_w}) \\
...
\mathrm{Intercept} \sim t(3,156,12) \\
A, G, A \colon G \sim t(3,0,12) \\
\sigma_L, \sigma_{A \colon L}, \sigma_{G \colon L}, \sigma_{A \colon G \colon L}, \sigma_{S_b}, \sigma_{S_g}, \sigma_{S_m}, \sigma_{S_w} \sim t(3,0,12) \\
\sigma \sim t(3,0,12) \\
\nu \sim gamma(2, 0.1) \\ 
R \sim \mathrm{LKJCorr} (2)
\end{split}
...
```{r, eval = FALSE}
model_formula = brms::bf(height ~ A*G + (A*G|L) + (1|gr(S, by=C_v)),
                         sigma ~ A + (A|L))
# 
# priors = c(brms::set_prior("student_t(3,156, 12)", class = "Intercept"),
#            brms::set_prior("student_t(3,0, 12)", class = "b"),
...
# saveRDS (model_het_group,"../models/8_model_het_group_2.RDS")
# model_het_group = readRDS ("../models/8_model_het_group.RDS")

```

And check out the print statement:
...
# sd(Intercept:C_vb)     4.54      0.87     3.04     6.43 1.00     3065     3593
# sd(Intercept:C_vg)     6.92      1.33     4.78     9.94 1.00     3064     3282
# sd(Intercept:C_vm)     1.57      0.41     0.80     2.43 1.00     2245     1786
# sd(Intercept:C_vw)     0.50      0.33     0.02     1.22 1.00     3230     3230
```

Which shows the standard deviation estimated for each speaker category. These are presented in figure below, which makes it clear that they do appear to differ by category. In the right plot below we see the speaker random effect according to this model. The use of category-specific standard deviations has had opposite effects on different speaker groups. If we compare the magnitude of the random effects of boys and girls presented in the figure above, the magnitudes are in general larger. In contrast we see that the speaker effects for women have been 'shrunk' to zero, more or less. Previously, our model use a compromise value for all group. $\sigma_S$. Now, since we use a different value for each speaker group we can allow $\sigma_{S_g}$ to be smaller and $\sigma_{S_g}$ to be larger, and this can affect 

```{r F87, fig.height = 3, fig.width = 8, fig.cap="--", cache = TRUE, echo = FALSE}
...
par (mfrow = c(1,2), mar = c(4,4,1,1))
layout (m = matrix(c(1,2,2),1,3))
brmplot (getsds(model_het_group)[7:10,], col = cols[2:5])
abline (h=2.36)
brmplot (ranef(model_het_group)$S[,,1], col = rep(cols[2:5], times =c(27,20,54,47)))
```
...
We are going to add another, more meta, question: Which should we report? As a first attempt, we can approach this from the perspective that the best model is the 'real' model. We can calculate elpd, for our newest model:
...
```
```{r F88, fig.height = 4, fig.width = 8, fig.cap="--", cache = TRUE, echo = FALSE}
