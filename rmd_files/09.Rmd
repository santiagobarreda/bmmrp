...
We fit a model that contains the category predictor, and our continuous predictor, but not the interaction between the two. This is the model presented in the right plot of \@ref(fig:F9-3).
...
For the sake of comparison, we also fit a model with speaker category predictors and no continuous predictor (`vtl`). As noted above, this is effectively a model with a bunch of horizontal lines, one for each group. This sort of model is seen in the middle plot of \@ref(fig:F9-3), and presented in \@ref(eq:99). 
