...
# save model
# saveRDS (model_single_line, '8_model_single_line.RDS')
```
```{r, include = FALSE}
model_single_line = readRDS ('../models/8_model_single_line.RDS')
```
...
We fit a model that contains the category predictor and also includes our continuous predictor, but not the interaction between the two (the model in the right plot of \@ref(fig:F83)):
...
model_many_intercept_one_slope =
  brm (height ~ C + vtl, 
       ...
       prior = c(set_prior("student_t(3, 160, 12)", class = "Intercept"),
                 ...
                 set_prior("student_t(3, 0, 12)", class = "sigma")))
# save model
# saveRDS (model_many_intercept_one_slope, '8_model_many_intercept_one_slope.RDS')

```
```{r, include = FALSE}
model_many_intercept_one_slope = 
  readRDS ('../models/8_model_many_intercept_one_slope.RDS')
```

For the sake of comparison, we will also fit a model with only group predictors and no continuous predictor (`vtl`). As noted above, this is effectively a model with a bunch of horizontal lines (slope = 0), one for each group (seen in the middle plot of \@ref(fig:F83)), and presented in \@ref(eq:89). 
...
model_many_intercept_no_slope =
 brm (height ~ C, 
       ...
       prior = c(set_prior("student_t(3, 160, 12)", class = "Intercept"),
                 ...
                 set_prior("student_t(3, 0, 12)", class = "sigma")))
# save model
# saveRDS (model_many_intercept_no_slope, '8_model_many_intercept_no_slope.RDS')
```
```{r, include = FALSE}
model_many_intercept_no_slope = readRDS ('../models/8_model_many_intercept_no_slope.RDS')
```
...
model_multi_slope =
  brm (height ~ C + vtl + vtl:C, 
       ...
       prior = c(set_prior("student_t(3, 160, 12)", class = "Intercept"),
                 ...
                 set_prior("student_t(3, 0, 12)", class = "sigma")))
# save model
# saveRDS (model_multi_slope, '8_model_multi_slope.RDS')

```
```{r, include = FALSE}
model_multi_slope = readRDS ('../models/8_model_multi_slope.RDS')
```
...
These coefficients are presented in figure \@ref(fig:F85). There appear to be gender-specific patterns in the intercept and slope coefficients between our groups. This pattern is evident when we use the line coefficients to draw each group-dependent line in the panel on the right. 


```{r F85, fig.width = 8, fig.height = 3.5, echo = FALSE, fig.cap = "(left) Group-specific intercepts. (middle) Group-specific slopes. (right) Lines for each group: boys (yellow), girls (green), men (red), and women (blue)."}
...
##  saveRDS (model_random_slopes_simple, '../models/8_model_random_slopes_simple.RDS')

```
```{r, include = TRUE, eval = FALSE}
# Or download it from the GitHub page:
model_random_slopes_simple = bmmb::get_model ('8_model_random_slopes_simple.RDS')
```
```{r, include = FALSE}
model_random_slopes_simple = readRDS ('../models/8_model_random_slopes_simple.RDS')
```
...
model_random_slopes_complex =  
  brms::brm (height ~ vtl*A*G + (vtl*A*G|L) + (1|S), data = height_exp, chains = 4, cores = 4,
       warmup = 1000, iter = 5000, thin = 4, prior = priors, family = "student")
##  saveRDS (model_random_slopes_complex, '../models/8_model_random_slopes_complex.RDS')

```
```{r, include = TRUE, eval = FALSE}
# Or download it from the GitHub page:
model_random_slopes_complex = bmmb::get_model ('8_model_random_slopes_complex.RDS')
```
```{r, include = FALSE}
...
```{r F88, fig.width = 8, fig.height = 3, fig.cap="Each plot shows responses from a single subject.", echo = FALSE, cache=TRUE}
...
Listener-dependent, group-specific lines are presented in figure \@ref(fig:87). Two things are clear. First, there is substantial variation in lines for different categories. Second, there is substantial variation in the lines representing each category for the different speakers. In both cases, this suggests that this model is preferable to `model_random_slopes_simple` since that model included only a single intercept and $VTL$ slope for each listener. 

```{r F89, fig.width = 8, fig.height = 5, fig.cap="Each plot shows responses from a single subject.", echo = FALSE, cache=TRUE}
...
The estimated simple effects are compared to group-specific intercepts and slopes in figure \@ref(fig:F810) and presented in table \@ref(tab:T81). 

```{r T81}
knitr::kable (simple_effects, caption="bla bla")
```

```{r F810, fig.width = 8, fig.height = 3, fig.cap="Each plot shows responses from a single subject.", echo = FALSE, cache=TRUE}
