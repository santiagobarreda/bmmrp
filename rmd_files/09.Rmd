...
To fit a model with a quantitative dependent variable and a single quantitative independent variable you need a data frame with at least two columns, both of which are quantitative variables (see section \@ref(c1-quantitative)), and which are being treated as numerical by R (rather than as strings or factors). It's hypothesized that listeners use information from speech acoustics to estimate a speaker's size (or height) based on the relationships between speech acoustics, height, and speaker VTL (see chapter 13). Based on this, we're going to try to predict apparent height given speaker VTL. The relevant variables from our data frame are:
...
The estimated simple effects are presented in table \@ref(tab:T9-1). In each case, we double the value of the effect so that these reflect differences between groups rather than the difference between the group and the main effect. You can confirm that the calues in the table match up with the information presented in the second and fourth plots in figure \@ref(fig:F9-9). 
...
We've specifically tried to avoid causal language so far, saying things like 'is associated with a change' rather than 'causes a change'. In earlier chapters, this was because we were modeling apparent height based on apparent spaker category. In this case, it is totally unclear based on the information we have what is causing what. For example, do listeners determine apparent age and then guess aparent height based on this? Do they estimate apparent height and then use this to estimate apparent age? Are both characteristics estimated simultaneously but in a related manner? So, in our models in previous chapters we were unsure about the causal direction of the effect and even of the process by which these might be related.

In the case of the models we fit in this chapter, we can be a little more sure about some causal directions, but not much more sure about the process. Its obvious that, if anything, speaker VTL influences apparent height rather than the other way around. We also know based on substantial previous research (see section X) that speaker VTL does predictably affect apparent height judgements in easily replicable ways in carefully controlled experiments. As a result, we do believe that longer VTLs (as measured via speech acoustics) 'cause' the reporting of taller apparent speakers. What we are less sure about is exactly how this occurs. 

For example, in this chapter we find that apparent age and gender affect affect apparent height and also affect the use of VTL. However, we still don't know if apparent age and gender influence apparent height, if the relation is the other way around, or what. We also know from previous research (see chapter 13) and our own models that speaker VTL is also very predictive of apparent gender and apparent age. As a result, we *can* say, somewhat definitively, that independent changes in speaker VTL cause changes in apparent height either directly or indirectly. However, we *cannot* say that the last model we fit (`model_random_slopes_complex`) is the *actual* model nor that we know much about how exactly VTL is used to estimate apparent height, gender, and age together, nor about the relationship between the processes underlying the reporting of these three apparent speaker characteristics. 
