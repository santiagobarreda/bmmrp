...
The function above may seem inscrutable, but it is very much scrutable. Recall that $z$ is the log-odds of observing a 'success', meaning that $e^z$ is the odds of observing of a success. The odds can be thought of as the ratio of the number of successes to the number of failures. When thought of in this way $e^z$ is the expected number of observed successes in a given situation. We can arbitrarily set the number of failures to 1 to make things simpler. To do this, we set $z=0$ (since $e^0=1$) for failures for all values of the dependent variables. This has no effect on the behavior of the function but does make the value of $e^z$ easily interpretable. For example odds of 3/1, 9/3, and 12/4 all mean the same thing, and by arbitrarily setting the number of failures to 1 we can say that the odds of a success is $e^z/1$ and the odds of a failure is $1/e^z$. 
  In order to convert counts of successes and failures to the probability of success, we need to divide the number of successes by the total number of observed outcomes, as seen in \@ref(eq:107a) below. Clearly, this is what the antilogit is doing in \@ref(eq:107b) above.

$$
\begin{equation}
P(y=1)=P(\mathrm{success}) = \frac{\mathrm{successes}}{\mathrm{failiures} + \mathrm{successes}}
(\#eq:107a)
\end{equation}
$$

The `bmmb` package contains a function called `antilogit` that will convert logits back into probabilities for you. Any number raised to the power of zero is 1. So $e^0=1$, meaning that when $z=0$ $p=0.5$, as shown in \@ref(eq:107).
...
plot (density (exp_data$vtl_original[exp_data$C_v=="b"]), xlim = c(11,16),
      ylim = c(0,1.3),lwd=2,col=cols[2],main="",xlab="Vocal-Tract Length (cm)")
polygon (density (exp_data$vtl_original[exp_data$C_v=="b"]),
       lwd=2,col=cols[2])
polygon (density (exp_data$vtl_original[exp_data$C_v=="g"]),
       lwd=2,col=cols[3])
polygon (density (exp_data$vtl_original[exp_data$C_v=="w"]),
       lwd=2,col=cols[5])
polygon (density (exp_data$vtl_original[exp_data$C_v=="m"]),
       lwd=2,col=cols[4])
