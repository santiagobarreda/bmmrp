...
Figure \@ref(fig:F10-1) shows the probability of observing a classification of 'adult' (i.e., 'man' or 'woman') for each speaker as a function of their VTL. Clearly, the perception of femaleness is related to speaker VTL: As VTL increases the probability of observing a female response decreases. This is not surprising as, in general, taller speakers tend to have longer VTLs and female speakers tend to be shorter than male speakers. So, listeners appear to be making the correct association between speaker gender and VTL and identifying speakers with shorter VTLs (who are likely also shorter in height) as female. 
  Unfortunately, we can't actually directly model variation in $p$ using lines as suggested in \@ref(eq:102) and in the bottom left plot of figure \@ref(fig:F10-1). This is because probabilities are bounded by zero and one, whereas our lines are not. This causes several problems. For example. Imagine we find that an effect causes an increase in probability of 0.2. If a group has a baseline probability of 0.9, its probability cannot increase by 0.2. This means an effect can't have the same magnitude given different underlying probabilities, which is a problem since our model parameter values are static. Ideally, we could model something that varies continuously along lines, but still allows us to predict variation in our $p$ parameter and dichotomous outcomes. 
...
The function above may seem inscrutable, but it is in fact very scrutable. First, note its structural similarity to the way that we convert counts of outcomes to probabilities presented in equation above. Recall that $z$ is the log-odds of observing a 'success', meaning that $e^z$ is the odds of observing of a success. The odds can be thought of as the ratio of the number of successes to the number of failures observed in some number of trials. As a result, $e^z$ can be thought of as the expected number of observed successes in a given situation. Based on the similarity to equation above, it seems as though we are fixing the expected 'count' of failures at 1, and this is in fact what logistic regression does. This is because for $N$ trials if you have $S$ successes then you *must* have $N-S$ failures. This means that the odds (or probabilities) of successes *and* failures cannot both be independently estimated. We ran into a similar problem previously when we discussed contrasts in chapter 5, and our discussion of why all levels of a factor cannot be independently estimated. Logistic regression arbitrarily sets the number of failures to 1 for all values of the dependent variables. This has no effect on the behavior of the function but does make the value of $e^z$ easily interpretable. For example odds of 3/1, 9/3, and 15/5 all mean the same thing, but imply different values of $e^z$ (i.e. $\log(3), \log(9), \log(12)$). By fixing the 'count' of failures at 1, we can say that the odds of a success is $e^z/1$ and the odds of a failure is $1/e^z$, knowing only a single value ($z$). 
  Thinking of $z$ as the logarithm of the expected number of successes relative to 1 expected failure may help interpret logits. For example, since $e^0=1$ a logit of 0 means we expect 1 success compared to 1 failure. This results in $p=0.5$ for a logit of 0, as shown in \@ref(eq:107).
...
And even put all of \@ref(eq:1011) inside the Bernoulli distribution directly as in \@ref(eq:1012).
...
- introduce predictor? space here.
...
```{r, eval = FALSE}
H = mean(exp_data$F[exp_data$G_v == "f"])
FA = mean(exp_data$F[exp_data$G_v == "m"])
```

Or more simply using: 

```{r, collapse = TRUE}
tapply(exp_data$F, exp_data$G_v, mean)
```
...
p1 = (tapply (exp_data$F, exp_data$G_v, mean))
...
p1 = bmmb::logit (tapply (exp_data$F, exp_data$G_v, mean))
...
Below is the function call we need to fit the model described in \@ref(eq:1019). Note that it is simply a logistic regression model, just one with a specific structure. 
