...
When $z$ is a negative value, $e^{-z}$ will be a fraction between 0 and 1. Obviously, this means that a success is less likely than a failure in these cases since we've fixed the number of failures at 1. For example if $z=-3$ then $e^{z}=e^{-3}=0.05$. This means that when $z<0$, we expect probabilities between 0.0 and 0.5, as in \@ref(eq:10-11).
...
Our boundaries are expressed relative to a mean VTL of zero. If we want them expressed relative to the true mean we need to add it back in. We can do this for our mean estimate and for our credible intervals too, and it is not necessary for the standard error. This might seem to violate our many (repeated) warnings about summarizing before combining. Why does this work? Because adding a number to all samples raises/lowers the sample mean and all the quantiles exactly by that number. As a result, adding that number to the mean and/or quantiles after calculating them has exactly the same effect. As for the standard error, adding a value to observations does not affect their variances so they remain unchanged. Below we add the original mean VTL back to our summary (excluding the standard error), and we see the category boundaries relative to values of VTL in the range of our real speakers. 
...
In most of the examples we've given so far, hit rates and false alarm rates have been balanced around probabilities of 0.5. This is equivalent to balancing out around values of 0 logits. When hits and false alarms balance out like this, this means that errors were equally likely to occur on both male and female speaker rounds. For example, a hit rate of 0.9 means listeners made a mistake on 10% of female trials, and a false alarm rate of 0.1 indicates the same error rate for male trials. But what if they don't balance out? 
