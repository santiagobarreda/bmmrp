...
**Logistic regression** is an approach to regression for predicting a dichotomous dependent variable. In terms of the components of the generalized linear model outlined above, logistic regression assumes a Bernoulli distribution for the random component (discussed above), and the *inverse logit* or *antilogit* function (discussed below) as its link function. The systematic components still consists of linear combinations of our independent variables, i.e., adding them up after multiplying by their respective model coefficients. A general description of logistic regression is given in \@ref(eq:10-4). 
...
p_{[i]} = \mathrm{logit}^{-1}(\theta_{[i]}) \\
\theta_{[i]} = Intercept + \beta_1 \times \mathrm{x}_{1[i]} + \dots + 
\beta_k \times \mathrm{x}_{k[i]}\\ 
...
\end{split}
(\#eq:10-4)
\end{equation}
...
The information in \@ref(eq:10-4) says the following: Our data ($y$) is a Bernoulli-distributed dichotomous variable with a $p$ parameter that varies on each trial. The $p$ parameter is the output of the inverse logit ($\mathrm{logit}^{-1}$) applied to our *linear predictor* $\theta$ (see section X). The linear predictor is assumed to be the sum of an intercept and the weighted combination of our predictors for predictors $x_1$ to $x_k$ and weights $\beta_1$ to $\beta_k$. 



We noted above that if you code successes and failures as 1 and 0 and find the average of a series of observations, you find the probability of observing a success in your observations. 


Actually though, we are estimating the *expected value* of *femaleness* for every individual speaker because the expected . 

Recall that for discrete variables, the expected value is equal to the sum of the product of possible outcomes times the probability of observing that outcome. Imagine a voice that was rated 10 times resulting in a responses of 1,1,0,1,0,0,1,1,1,1, where 1 represents female. The expected 
...
We will to discuss the inverse logit function in detail, however, to talk about this link function we need to model dichotomous data, it helps to talk about *logits* first. **Logits** are log-odds, the logarithm of the odds of a success. The odds of a success is defined as:
...
So, odds of 3/1 imply a probability of 0.25 (3 / (3+1)). We can define odds in a more formal manner by expressing them in terms of the probability of a success($p$) and the probability of a failure ($1-p$) as in \@ref(eq:10-6).
...
Our logistic regression models express predictions in logits, and our data generating distributions (Bernoulli or binomial) require a parameter $p$ that represents a probability. This means that the link function for our logistic regression models needs to convert logits to probabilities. The function that does this is called the **logistic function**, or sometimes the **antilogit function**. The logistic function is presented in \@ref(eq:10-6), for values of $z$ ranging continuously from positive to negative infinity, where $e$ is the mathematical constant that serves as the base for natural logarithms.
...
The function above may seem inscrutable, but it is in fact very scrutable. First, note its structural similarity to the way that we convert counts of outcomes to probabilities presented in equation above. Recall that $z$ is the log-odds of observing a 'success', meaning that $e^z$ is the odds of observing of a success. The odds can be thought of as the ratio of the number of successes to the number of failures observed in some number of trials. As a result, $e^z$ can be thought of as the expected number of observed successes in a given situation. Based on the similarity to equation above, it seems as though we are fixing the expected 'count' of failures at 1, and this is in fact what logistic regression does. This is because for $N$ trials if you have $S$ successes then you *must* have $N-S$ failures. This means that the odds (or probabilities) of successes *and* failures cannot both be independently estimated. We ran into a similar problem previously when we discussed contrasts in chapter 5, and our discussion of why all levels of a factor cannot be independently estimated. Logistic regression arbitrarily sets the number of failures to 1 for all values of the dependent variables. This has no effect on the behavior of the function but does make the value of $e^z$ easily interpretable. For example odds of 3/1, 9/3, and 15/5 all mean the same thing, but imply different values of $e^z$ (i.e. $\log(3), \log(9), \log(12)$). By fixing the 'count' of failures at 1, we can say that the odds of a success is $e^z/1$ and the odds of a failure is $1/e^z$, knowing only a single value ($z$). 

Thinking of $z$ as the logarithm of the expected number of successes relative to 1 expected failure may help interpret logits. For example, since $e^0=1$ a logit of 0 means we expect 1 success compared to 1 failure. This results in $p=0.5$ for a logit of 0, as shown in \@ref(eq:10-7).
...
When $z$ is a negative value, $e^{-z}$ will be a fraction between 0 and 1. Obviously, this means that a success is less likely than a failiure in these cases since we have fixed the number of failures at 1. For example if $z=-3$ then $e^{z}=e^{-3}=0.05$. This means that when $z<0$, we expect probabilities between 0.0 and 0.5, as in \@ref(eq:10-8).
...
$$
\begin{equation}
p = \mathrm{logit}^{-1}(Intercept + VTL \times \mathrm{vtl}) =  \frac{e^{(Intercept + VTL \times \mathrm{vtl})}}{1 + e^{(Intercept + VTL \times \mathrm{vtl})}}
(\#eq:10-11)
\end{equation}
$$

And even put all of \@ref(eq:10-11) inside the Bernoulli distribution directly as in \@ref(eq:10-12).

$$
\begin{equation}
F \sim \mathrm{Bernoulli} \left (\frac{e^{(Intercept + VTL \times \mathrm{vtl})}}{1 + e^{(Intercept + VTL \times \mathrm{vtl})}} \right ) 
(\#eq:10-12)
\end{equation}
$$
...
In the previous chapter we tried to predict apparent height using speaker VTL, apparent age and apparent gender.This time, we're going to try to predict the perception of *femaleness* based on speaker VTL an apparent age, using a logistic regression.
