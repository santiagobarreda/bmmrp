...
s3d = scatterplot3d (tmp, color = bmmb::cols[2:5][factor(agg_data[,3])],
                     pch=16, angle = 55,type = "p",ylim=c(90,310), cex.symbols=rev(ptcex)*.9)
my.lm <- lm(height ~ f0+vtl, data = agg_data)

s3d = scatterplot3d (agg_data[,-3], color = bmmb::cols[2:5][factor(agg_data[,3])],
...
z <- outer(x, y, f1)
persp(x, y, z, col = cols[2],theta = 0,zlim = c(-3,3))
persp(x, y, z, col = cols[2],theta = -90,zlim = c(-3,3))
persp(x, y, z, col = cols[2],theta = 45,zlim = c(-3,3))

z <- outer(x, y, f2)
persp(x, y, z, col = cols[3],theta = 0,zlim = c(-3,3))
persp(x, y, z, col = cols[3],theta = -90,zlim = c(-3,3))
persp(x, y, z, col = cols[3],theta = 45,zlim = c(-3,3))

z <- outer(x, y, f3)
persp(x, y, z, col = cols[4],theta = 0,zlim = c(-3,5))
persp(x, y, z, col = cols[4],theta = -90,zlim = c(-3,5))
persp(x, y, z, col = cols[4],theta = 45,zlim = c(-3,5))
...
$$
\begin{equation}
z =  \mathrm{a} + (\mathrm{b} \times x) + (\mathrm{c} \times y) + (\mathrm{c} \times xy)
(\#eq:11_3)
\end{equation}
$$
...
c_{[i]} = f0 + VTL \colon A + f0 \colon G + f0 \colon A \colon G + f0 \colon A \colon L_{[L_{[i]}]} + f0 \colon G \colon L_{[L_{[i]}]} + f0 \colon A \colon G \colon L_{[L_{[i]}]} + f0 \colon L_{[L_{[i]}]}  \\ \\
\mathrm{Priors:} \\ 
...
model_height_vtl_f0_int = bmmb::get_model ('11_model_height_vtl_f0_int.RDS')
```
```{r, include = FALSE, eval = FALSE}
# saveRDS (model_height_vtl_f0_int, '../models/11_model_height_vtl_f0_int.RDS')
model_height_vtl_f0_int = readRDS ('../models/11_model_height_vtl_f0_int.RDS')
```
...
### Some advantages of Bayesian multilevel models
...
At this point we have enough model components to build very large and complicated models, like the one we fit above. Traditionally, models with many predictors have presented three general problems. First, a model with 'too many' predictors may return spurious values for the 'extra' predictors. Sometimes these spurious values are difficult to distinguish from the 'real' parameters, leading to incorrect conclusions. Second, the model may not be able to fit/converge on a solutions, meaning you can't get the model coefficients. Regardless of the approach to parameter estimation, more-complicated models make it more and more difficult for any estimation to 'find' the optimal parameter values given the data. Third, it can be difficult to interpret a model with *hundreds* (or thousands) of parameters. This becomes especially problematic when considered together with problem (1), that some of these values may simply represent noise. In this section we're going to discuss how working with multilevel Bayesian models can naturally help us with problems (1) and (2) above. In the following section we will discuss how our models can also help us resolve the third problem above.  
  ...
  Below we fit a model equivalent to `model_height_vtl_f0_int` using `lmer`.
...
bmmb::brmplot (fixef (model_height_vtl_f0_int)[-1,], ylim = c(-7,10),las=2)
...
vars_final = bmmb::getsds(model_height_vtl_f0_int)
...
par (mar = c(.5,4,.5,1))
bmmb::brmplot (ranef(model_height_vtl_f0_int)$L[,,4],labels="", col = 2)
...
corrs_brms = bmmb::getcorrs(model_height_vtl_f0_int, "L")
...
brmplot (ranef (model_height_vtl_f0_int)$L[,,"Intercept"], col = bmmb::cols[9])
brmplot (ranef (model_height_vtl_f0_int)$L[,,"vtl:G1"], add = TRUE, 
...
brms::VarCorr(model_height_vtl_f0_int)

brms::VarCorr(model_height_vtl_f0_int)[["L"]][["sd"]]
```

We can also get information regarding our error standard deviation using `Varcorr` as seen below:

```{r, cache = TRUE, collapse = TRUE}
brms::VarCorr(model_height_vtl_f0_int)$residual$sd
```
...
```{r}
getsds (model_height_vtl_f0_int)
```
...
listener_intercepts = ranef(model_height_vtl_f0_int, summary = FALSE)[["L"]][,,"Intercept"]
...
banova_height_vtl_f0_finite = banova (model_height_vtl_f0_int, superpopulation = FALSE)
banova_height_vtl_f0_super = banova (model_height_vtl_f0_int, superpopulation = TRUE)
```
...
banovaplot (banova_height_vtl_f0_1[-2,], las = 2,line=FALSE,labels = "",
            yaxs="i", ylim = c(0,10))
abline (h=0)
box()
banovaplot (banova_height_vtl_f0_2[-2,], las = 2, line=FALSE,yaxs="i",
            ylim = c(0,10))
abline (h=0)
box()
...
`height ~ f0 + vtl + A*G + vtl:A1`




```
height ~ vtl + f0 + A + G + A1:G1 + vtl:f0 + vtl:A1 + 
         vtl:G1 + f0:A1 + f0:G1 + vtl:f0:A1 + vtl:f0:G1 + 
         vtl:A1:G1 + f0:A1:G1 + vtl:f0:A1:G1
```
...
```
RTtrunc ~  1 + vtl + f0 + C + SP + SC + PC + SPC + 
...
We fit a reduced model based on 
...
We see that the fixed effects shared in common are very similar. 

```{r}
brmplot (fixef (model_height_vtl_f0_int)[-1,], ylim = c(-6,10))
brmplot (xs = c(2,1,3,4,10,6),fixef (model_height_vtl_f0_reduced)[-1,],
...
Below we add our dependent variable `F`. This variable
...
We will use the same priors we have been using for our logistic models. We will omit our model specification since it is quite large and very similar to the one above for `model_height_vtl_f0` and those presented in chapter 10. 
...
```
```{r, include = FALSE, eval = FALSE}
# saveRDS (model_gender_vtl_f0, '../models/11_model_gender_vtl_f0_2.RDS')
model_gender_vtl_f0 = readRDS ('../models/11_model_gender_vtl_f0_2.RDS')
```

We can consider the model fixed effects using `brmplot` in figure \@ref(fig:11_10)., and beside that we plot 
...
```{r F11_10, fig.height = 3, fig.width = 8, fig.cap='--', echo = FALSE}
...
```{r F91, fig.width = 8, fig.height = 3.5, fig.cap = "(left) Average perceived height for each speaker as a function of average f0. (middle) The same relationship as the left but only males are plotted. (right) Only females are plotted.", echo = FALSE, eval = FALSE}
...
par (mfrow = c(1,2), mar = c(4,2,1,1), oma = c(1,4,0,0))
...
loo_compare (model_height_vtl_f0, model_height_vtl_f0_int,model_random_slopes_complex)
...
```{r, eval = FALSE}
qq = banova (model_height_vtl_f0_int, superpopulation = TRUE)
