...
```{r F11_13, fig.width = 8, fig.height = 3.5, fig.cap = "(left) Average perceived height for each speaker as a function of average f0. (middle) The same relationship as the left but only males are plotted. (right) Only females are plotted.", echo = FALSE, eval = FALSE}
...
In each case, a posterior predictive check may help understand the issues, as can inspecting the distribution of pareto k statistics. 





```{r F11_14, fig.width = 8, fig.height = 3, fig.cap = "(left) Average perceived height for each speaker as a function of average f0. (middle) The same relationship as the left but only males are plotted. (right) Only females are plotted.", echo = FALSE, eval = FALSE}
...
plot (model_gender_vtl_f0$criteria$loo$diagnostics$pareto_k, ylab="", 
      ylim = c(0,1.5),col=bmmb::cols[1+mod_cat],pch=16, xaxt='n')
plot (model_gender_vtl_f0_reduced$criteria$loo$diagnostics$pareto_k, ylab="", 
      ylim = c(0,1.5),col=bmmb::cols[1+mod_cat],pch=16, yaxt='n', xaxt='n')
```

```{r}
preds = predict (model_gender_vtl_f0)
preds_reduced = predict (model_gender_vtl_f0_reduced)
```
...
plot (preds[,1], model_gender_vtl_f0$criteria$loo$diagnostics$pareto_k,
      col=bmmb::cols[1+mod_cat],pch=16, ylim = c(0,1.5))
plot (preds_reduced[,1], model_gender_vtl_f0_reduced$criteria$loo$diagnostics$pareto_k,
      col=bmmb::cols[1+mod_cat],pch=16, ylim = c(0,1.5))
```
...
model_gender_vtl_f0_conservative =
  brm (F ~ vtl*f0*A + (vtl*f0*A|L) + (1|S), data=height_exp, 
       chains=4, cores=4, family="bernoulli", 
       warmup=1000, iter = 5000, thin = 4,  
...
```{r, include = TRUE, eval = FALSE}
# Or download it from the GitHub page:
model_gender_vtl_f0_conservative = bmmb::get_model ('11_model_gender_vtl_f0_conservative.RDS')
```
```{r, include = FALSE, eval = TRUE}
# saveRDS (model_gender_vtl_f0_conservative, '../models/11_model_gender_vtl_f0_conservative.RDS')
model_gender_vtl_f0_conservative = readRDS ('../models/11_model_gender_vtl_f0_conservative.RDS')
```


```{r, eval = FALSE}
model_gender_vtl_f0_conservative = brms::add_criterion(model_gender_vtl_f0_conservative,"loo")
```
