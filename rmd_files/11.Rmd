...
```{r F11-1, fig.height = 3, fig.width = 8, fig.cap='(left). (middle). (right).', echo = FALSE, cache = TRUE}
...
It may be easier to see what we mean by considering figure \@ref(fig:11.2), which attempts to present our point in three dimensions. The left plot of figure \@ref(fig:11.2) corresponds to the left plot in figure \@ref(fig:F11-1), while the right plot of figure \@ref(fig:11.2) corresponds to the middle plot in figure \@ref(fig:F11-1). If we want to predict apparent height based on speaker f0 and VTL, that is basically asking: Can we predict the height of a point in our 3-dimensional space based on its x and y-axis location? 

```{r F11-2, fig.height = 5, fig.width = 8, fig.cap='A pairs plot of the continuous variables in our data, showing different sorts of relationships between our variables.', echo = FALSE, cache = TRUE}
...
When we have two quantitative predictors, our models predict values along **planes** rather than lines. For example, imagine the points in figure \@ref(fig:11.2) were floating in the room with you, arranged just as in the figure. You are given a large flat board (a *plane*) and asked to find the "best" position for the board. When we fit lines, we preferred those that tended to minimize the residuals, the y-axis distance of the points to the line. The same principle holds when we fit models with two (or more) quantitative predictors. A Regressions model that includes two continuous predictors tries to find the 'best' plane that passes through your points in the three-dimensional space. In general, the 'best' plane is the one that minimizes the distance from each point to the surface of the plane along the axis representing the dependent variable. Though it's a bit more complicated than this for multilevel models, this is still basically what's happening.  
  Planes are specified by three parameters, a slope for each dimensions and an intercept. In \@ref(11-1) we that the height of the plane along the $z$ axis is determined by an intercept ($a$) plus the product of the x-axis coordinate and its slope ($b$) and the product of the y-axis coordinate and its slope. When we fit a model that includes two quantitative predictors, the model represents the best planes using their $a$, $b$, and $c$ parameters. 
...
In figure \@ref(fig:11.3) we can see a comparison of a plane and two saddle shapes. In the middle left plot we can see the analogy of the plaza with the sloping floor. If we were walking "into" the plot the ground would first be sloping down left to right, but gradually changes so that it it sloping up left to right by the end of the surface. 

```{r F11-3, fig.height = 6, fig.width = 8, fig.cap='A pairs plot of the continuous variables in our data, showing different sorts of relationships between our variables.', echo = FALSE, cache = TRUE}
...
We can combine a plane and a saddle shape, as in the bottom row of figure \@ref(fig:11.3). The general formula for such a shape is presented below, and we've placed the terms in parentheses just to make it easier to interpret the equation. 
...
What does any of this mean for our regression models? We discussed above that models with two continuous predictors model variation in the dependent variable along planes. The 'interaction' between quantitative variables in regression models is represented by the **cross-product**, the multiple, of the two variables. So, the interaction of quantitative predictors results in a saddle shape as defined in \@ref(eq:11.2). When our models include effects for $x$, $y$, *and* their interactions, we are effectively modeling a surface that combined a plane and a saddle shape, as in \@ref(eq:11.3). 
...
```{r F11-4, fig.height = 3.5, fig.width = 8, fig.cap='', echo = FALSE, cache = TRUE}
...
```{r, F11-5, fig.height = 6, fig.width = 8, fig.cap='', echo = FALSE, cache = TRUE}
...
```{r F11-7, fig.height = 4.5, fig.width = 8, fig.cap='', echo = FALSE}
...
```{r F11-9, fig.height = 4, fig.width = 8, fig.cap=' -- ', echo = FALSE, cache = TRUE}
...
```{r F11-10, fig.height = 4, fig.width = 8, fig.cap=' -- ', echo = FALSE, cache = TRUE}
...
```{r F11-11, fig.height = 3, fig.width = 8, fig.cap='--', echo = FALSE, cache = TRUE}
...
```{r F11-12, fig.width = 8, fig.height = 3.5, fig.cap = "(left) Average perceived height for each speaker as a function of average f0. (middle) The same relationship as the left but only males are plotted. (right) Only females are plotted.", echo = FALSE, cache = TRUE}
...
curve ((-b_all*x - a_all) / (d_all*x+c_all), from = -6, to = -.53, add = TRUE,lwd=2,col=bmmb::cols[7])
curve ((-b_all*x - a_all) / (d_all*x+c_all), from = -.52, to = 6, add = TRUE,lwd=2,col=bmmb::cols[7])
...
plot (aggd$vtl,aggd$f0, cex =1.2, col = bmmb::cols[c(2:5)][factor(aggd$C)], 
      pch=16,lwd=2, xlab = "",xlim = c(-4,4),ylim = c(-2,2),yaxt='n',
      ylab="Height (inches)")
grid()
points (aggd$vtl, aggd$f0, cex =1.2, pch=16,lwd=2,
      col = bmmb::cols[c(2:5)][factor(aggd$C)])
...
model_gender_vtl_f0_reduced =
  brm (F ~ (vtl+f0)*A + ((vtl+f0)|L) + (1|S), data=height_exp, 
       chains=4, cores=4, family="bernoulli", 
       warmup=1000, iter = 5000, thin = 4,  
       prior = c(set_prior("student_t(3, 0, 3)", class = "Intercept"),
                 set_prior("student_t(3, 0, 3)", class = "b"),
...
```{r F11-13, fig.width = 8, fig.height = 3, fig.cap = "(left) Average perceived height for each speaker as a function of average f0. (middle) The same relationship as the left but only males are plotted. (right) Only females are plotted.", echo = FALSE, cache = TRUE}
...
However, we got the following error message we adding the `loo` criterion to our model:

`Warning: Found 142 observations with a pareto_k > 0.7 in model 'model_gender_vtl_f0'. It is recommended to set 'moment_match = TRUE' in order to perform moment matching for problematic observations.`
...
```{r F11-14, fig.width = 8, fig.height = 3, fig.cap = "(left) --. (right) --.", echo = FALSE, cache = TRUE}

################################################################################
### Figure 11-14
################################################################################
point_col = tapply (height_exp$C_v, height_exp$S,findmode)
...
p_preds_reduced = predict (model_gender_vtl_f0_reduced)
```
...
```{r F11-15, fig.width = 8, fig.height = 3, fig.cap = "(left) --. (middle) --. (right) --.", echo = FALSE, cache = TRUE}
...
#par (mfrow = c(1,2), mar = c(4,.5,1,.5), oma = c(0,4,0,0))
par (mfrow = c(1,3), mar = c(4,4,1,1))

plot (p_preds[,1], model_gender_vtl_f0$criteria$loo$diagnostics$pareto_k,
      col=bmmb::cols[1+point_col],pch=16, ylim = c(0,1.5))

#plot (p_preds_reduced[,1], model_gender_vtl_f0_reduced$criteria$loo$diagnostics$pareto_k,
#      col=bmmb::cols[1+point_col],pch=16, ylim = c(0,1.5))

#aggd = aggregate (pareto_k ~ f0_original + vtl_original + S, data = height_exp, FUN = mean)
aggd = aggregate (pareto_k ~ f0 + vtl + S + A_v, data = height_exp, FUN = mean)
aggd = aggd[aggd$A_v=="a",]

tmp = height_exp[height_exp$A_v=="a",]
point_col = tapply (tmp$C_v, tmp$S,findmode)
point_col = as.numeric(factor(point_col))+2

#plot(aggd[,2:1], cex = aggd$pareto_k*4, col = cols[1+point_col], pch=1,lwd=2,
#     xlim = c(11,16.5), ylim = c(80,310), ylab="f0 (Hz)", xlab = "Vocal-tract length (cm)")
plot(aggd[,2:1], cex = aggd$pareto_k*4, col = cols[1+point_col], pch=16,lwd=2,
     xlab = "Expected Probbility of a Female Response", xlim = c(-2.2,3), 
     ylim = c(-1.2,1.1), ylab = "Pareto k")


#curve ((-b_adult*x - a_adult) / (d_adult*x+c_adult), from = -6, to = -c_adult/d_adult, add = FALSE,
#       lwd=2, lty=2,col=bmmb::cols[10], xlim = c(-2.2,2.5), ylim = c(-1.2,1.1))
#curve ((-b_adult*x - a_adult) / (d_adult*x+c_adult), from = -c_adult/d_adult, to = 6, add = TRUE,
#       lwd=2, lty=2,col=bmmb::cols[10])

#curve ((-b_adult*x - a_adult - 5) / (d_adult*x+c_adult), from = -6, to = -c_adult/d_adult, add = TRUE,
#       lwd=2, lty=2,col=bmmb::cols[10])

curve ((-b_adult*x - a_adult) / (d_adult*x+c_adult), from = -6, 
       to = -c_adult/d_adult, add = FALSE, lwd=2, lty=1,col=1, xlim = c(-2.2,3), ylim = c(-1.2,1.1))
curve ((-b_adult*x - a_adult) / (d_adult*x+c_adult), from = -c_adult/d_adult, 
...
for (i in seq(-62,-2,2)){
  curve ((-b_adult*x - a_adult - i) / (d_adult*x+c_adult), from = -6, 
         to = -c_adult/d_adult, add = TRUE, lwd=1, lty=1,col=bmmb::cols[5])
  curve ((-b_adult*x - a_adult - i) / (d_adult*x+c_adult), from = -c_adult/d_adult, 
       to = 6, add = TRUE, lwd=1, lty=1,col=bmmb::cols[5])
}

for (i in seq(2,62,2)){
  curve ((-b_adult*x - a_adult - i) / (d_adult*x+c_adult), from = -6, 
         to = -c_adult/d_adult, add = TRUE, lwd=1, lty=1,col=bmmb::cols[4])
  curve ((-b_adult*x - a_adult - i) / (d_adult*x+c_adult), from = -c_adult/d_adult, 
       to = 6, add = TRUE, lwd=1, lty=1,col=bmmb::cols[4])
  
}
points(aggd[,2:1], cex = 1, col = 1, pch=1,lwd=2)
...
```{r F11-16, fig.width = 8, fig.height = 5, fig.cap = "(left) --. (right) --.", echo = FALSE, cache = TRUE}
...
aggd = aggregate (cbind ( height, A=="a", G=="f", vtl,f0, vtl) ~ S + A_v + G_v, 
                      data = height_exp, FUN = mean)
aggd = aggd[aggd$A_v=='a',]

a = listener_coefficients[1:15,1]
b = listener_coefficients[16:30,1]
c = listener_coefficients[31:45,1]

par (mar = c(.2,.2,.2,.2), oma = c(3,3,.5,.5), mfrow = c(4,4))

plot (aggd$vtl,aggd$f0, cex =1.2, col = bmmb::cols[c(4:5)][factor(aggd$G_v)], 
      pch=16,lwd=2, xlab = "",ylab="f0 (cm)",xaxt = 'n')
grid()

for (i in 1:15)
  curve ((-b[i]*x - a[i]) / (c[i]), from = -3, to = 3, add = TRUE,lwd=2,col=bmmb::cols[7],xaxt='n')
...
for (i in 1:15){
  ...
  tmp = height_exp[height_exp$L==i & height_exp$A=='a',]
  plot (tmp$vtl,tmp$f0, cex =1.2, col = bmmb::cols[4:5][factor(tmp$G)],
        pch=16,lwd=2, xlab = "",ylab="f0 (cm)",xaxt=xaxt,yaxt=yaxt,xlim = c(-1.1,2.6),ylim = c(-1.2,1))
  grid()
  curve ((-b[i]*x - a[i]) / (c[i]), from = -3, to = 3, add = TRUE,lwd=2,col=bmmb::cols[7])
