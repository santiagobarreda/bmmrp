...
```{r F11-12, fig.width = 8, fig.height = 3, fig.cap = "(left) Average perceived height for each speaker as a function of average f0. (middle) The same relationship as the left but only males are plotted. (right) Only females are plotted.", echo = FALSE, cache = FALSE}
...
```{r F11-16, fig.width = 8, fig.height = 5, fig.cap = "(left) --. (right) --.", echo = FALSE, cache = FALSE}
...
for (i in 1:15){
  ...
  plot (tmpp$vtl,tmpp$f0, cex =1.2, col = bmmb::cols[2:5][factor(tmpp$C)],
        ...
        xlim=c(-2.5,2.5),ylim=c(-1.1,1.1))
  grid()
  curve ((-b[i]*x - a[i]) / (c[i]), from = -3, to = 3, add = TRUE,
         lwd=2,col=bmmb::cols[8])
  curve ((-bc[i]*x - ac[i]) / (cc[i]), from = -3, to = 3, add = TRUE,
