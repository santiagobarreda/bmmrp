...
[@@ NHS: I misread this as "Chapter pre-*crap*"]

--

## Models with multiple quantitative predictors

Our regression models so far have only involved at most a single quantitative predictor. This means that the relationship between the dependent variable and our predictor formed surfaces with a single dimension: Lines. In the left and middle plots in figure \@ref(fig:F11-1) we see the linear relationships between speaker vocal-tract length (VTL) and f0 (voice pitch) with apparent speaker height. In each case, the expected value for the y-axis variable is the value of the line at each x-axis location. The residual, the error in the prediction of each observation, is the vertical distance between the line the observation.   
...
[@@ NHS: why not standardize using 1 SD for each variable?]
...
For example, the first line alone would be:
...
[@@ NHS: I'm not sure about the argument about "bouncing" off of boundaries here - Bayesian models can definitely have problems with hard limits on parameters, too. There may be advantages to sampling from a posterior rather than trying to find a point estimate in these situations, but it's not just because they avoid the boundary altogether. I think it would be due more to the fact that you can get a set of parameter values relatively near the "true"/optimal value (maybe this is what you meant with the 'bouncing' reference, now that I think about it some more). I actually think that shrinkage and priors is a key element with the boundary issue, too, since shrinkage/priors helps stabilize estimates (i.e., reduce the variance of estimates) at the cost of some degree of bias]
...
Which warns us that some of the variance components we are trying to estimate are quite small. Because `brm` simply samples from the posterior and doesn't try to find *the* best model parameters, these issues are minimized. [@@ NHS: just noting again that we should talk/think more about why exactly Bayesian models and/or brms specifically do well with, e.g., small variance estimates - I wonder if it's actually just a matter of under-the-hood transformations to avoid boundary issues? maybe you should email Paul Bürkner and ask him what is going on in a case like this]
...
The main reason to do it the way way we've shown above is because it can be done easily for all models, and you still get very useful information from the analysis. If you do want to investigate the variation associated with entire clusters of multiple predictors at a time, please see the readings referred to at the top of section \@ref(c11-BANOVA). Although the complete method is a straightforward extension of what we've discussed here, there are a few important additional details (e.g., the need to 'recover' missing parameters, the need to calculate 'degrees of freedom', etc.). 
...
As with our height model, we'll leave more detailed research questions for chapter 13. For now we are only interested in two questions based on the methods we discussed in section \@ref(c11-BANOVA): 

Q1) Can we use what we've learned so far to fit and evaluate a logistic regression model with two quantitative predictors?

Q2) Can we extend what we've learned so far to use our logistic model to classify speakers along two stimulus dimensions (f0 and VTL)?
...
We'll use the same priors we used for our logistic models last chapter. Since we scaled f0 so that a change of 1 corresponds to 100 Hz, a 1 unit change in f0 this value represents about the average difference in f0 between adult males and females. For this reason, we think it's reasonable to use a prior of the same magnitude as we used for our VTL parameter. 

We will omit our model specification since it is quite large and very similar to the presented in chapter 10 for `model_gender_vtl`. However we can say what it's doing in general. We are predicting the perception of femaleness using planes, or potentially saddle shapes, that vary as a function of speaker VTL and f0. These shapes can vary arbitrarily, in their slopes along each dimension, degree of curvature, and intercepts, as a function of apparent height. *All* of the aforementioned characteristics can vary in a listener dependent way, and intercepts can vary in a speaker dependent way. If the preceding sentences make no sense to you, you should review the description of the linear model in section \@ref(c11-description-1). All we've done here is briefly say the same thing we said in more detail in that section. 
...
We can also add up appropriate combinations of the fixed effects to calculate separate $a, b, c$ and $d$ parameters for apparent adults and for apparent children, as seen below. Note that in each case we combine the necessary parameters first and then find the average across the samples of the parameter. 
...
In the hopes of finding a model that 'makes more sense', we're going to drop the cross-product from our model. Below we fit a reduced model that is otherwise the same as our initial model, but without the interaction between f0 and VTL. 
...
We find the $a,b,c$ parameters for the response plane given our model, calculating the overall (marginal) plane, a plane for apparent children, and another for apparent adults. There is no $d$ parameter this time since there is no cross-product term in this model. 
...
The comparison suggests the model that includes the interaction does a better job of explaining our data. The differences between the two models is large-ish, but just under two standard errors in magnitude and so is not terribly reliable. 
...
When we add the `loo` criterion to our model, `brms` (and `stan`) don't actually fit a new model for every left out data point (i.e. a 'real' leave-one-out cross validation). Instead, there's a way to approximate what the model *would* look like had it actually been refit without each data point. This approximation does using something called **Pareto smoothed importance sampling** (**PSIS**). The Pareto-k ($\hat{k}$, `pareto_k`) statistic is a diagnostic for this estimation method that helps you check the assumptions underlying the leave-one-out approximation (cite, Vehtari, Gelman and Gabry, 2017). 
