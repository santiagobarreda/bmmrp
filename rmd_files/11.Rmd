...
In this chapter we introduce models with multiple quantitative predictors, and interactions between them. After that, we will have covered the basic modeling concepts necessary to fit most models. The models you fit to you own data will include some combination of the elements covered in the previous chapters, and can potentially result in large models with hundreds of estimated parameters. Traditionally, models with many estimated parameters have had three general problems:

  1) A model with 'too many' predictors may return spurious values for the 'extra' predictors. We will vaguely define 'extra' predictors as those which have no statistical association with your dependent variable, and which do not appreciably improve your model in any way. Sometimes these spurious values are difficult to distinguish from the 'real' parameters, leading to incorrect conclusions. 
  2) Second, the model may not be able to fit/converge on a solutions, meaning you can't get good estimates for the model coefficients. Regardless of the approach to parameter estimation, more-complicated models make it more difficult for any estimator to 'find' the optimal parameter values given the data. 
  3) It can be difficult to interpret a model with *hundreds* (or thousands) of parameters. This becomes especially problematic when considered together with problem (1), that some of these values may simply represent noise. 
...
Before continuing, we should note that designs with many quantitative predictors, factors, and interactions between these can result in very complicated models which then have to be interpreted. However, the researcher has a big role to play in the complexity of the analysis they are faced with. Once when Santiago was buying a backpack for traveling, he was looking for the biggest backpack possible. One of the reviews said "1/5 stars, it was way too heavy when I filled it all the way up with my stuff". However, if we fill a backpack up with many heavy things, it doesn't seem fair to blame the backpack when it becomes difficult to carry. In the same way, if you are faced with a complex model that you then need to interpret, you shouldn't blame the model, `brms`, or us for your predicament. 
...
In order to avoid a situation where you end up with data you can't analyze or a model you can't interpret, it's worth considering the following questions before advancing to the data collection stage of your experiment: 
  ...
  * How would different results manifest in the model parameters?
...
```{r F11-2, fig.height = 4.5, fig.width = 8, fig.cap='(left) A three-dimensional plot of the variables presented in the figure above. (right) The same as the left plot but the cube has been rotated 90 debrees counter-clockwise.', echo = FALSE, cache = TRUE}
...
In general, interaction terms are parameters that allow for the effect of a predictor to vary according to the value of some other predictor. When it comes to quantitative predictors, an interaction means that the slope of each predictor continuously increases/decreases as a function of the value of the *other* predictor. Consider the equation below which omits the term $(c \times y)$ for the sake of simplicity. If we factor out the $x$ form the second and third terms on the right. When we do this, we can see that the slope for the $x$ dimension, $b$, will vary as a function of the value of $y$ and the interaction coefficient $d$.
...
Imagine a case where the slope ($b$) along the $x$ dimension is fixed at 2 and the interaction term ($d$) is equal to zero. In this case the is no interaction between $x$ and $y$. When this happens the slope of $x$ is constant and does not change based on the value of $y$, as in \@ref(eq:11-4). 
...
So, the interaction of quantitative predictors results in a saddle shape as defined in \@ref(eq:11-6). When our models include effects for $x$, $y$, *and* their interactions, we are effectively modeling a surface that combines a plane and a saddle shape, as in \@ref(eq:11-2). In figure \@ref(fig:F11-3) we can see a comparison of a plane in the top row and a saddle shape in the middle row. In the middle row on the left we can imagine that if we were walking 'into' the plot the ground would first be sloping down left to right. However, as we proceeded into the plot the ground would gradually change so that it it sloping up from left to right further into the figure. In other words, the left-to-tight slope would gradually increase as we walk further into the plot.  
...
And the lines below that would have described $a$, $b$, $c$, and $d$ as the sum of the appropriate model parameters. 
...
Whereas figure \@ref(fig:F11-4) showed the similarity of our `lmer` and `brm` models, figure \@ref(fig:F11-5) presents some ways in which these models differ. In the top row we see a comparison of the 'random effects' standard deviation estimates provided by our two models, and the error terms (i.e., `sigma` or $\sigma$) estimated by our two models. For example, the standard deviation of the listener f0 'random effect' (`f0:L`, $\sigma_{f0 \colon L}$) represents the amount of variation around zero between the listener-specific effects for f0 in each model. As we can see, the models provide reasonably similar standard deviation estimates for many, if not most, parameters. However, note that the `brm` models provide credible intervals for all parameters, while the `lmer` model only provides point estimates for these.
...
Each of these batches has a thematic or semantic link; these are not just parameters grouped at random. When we break up variation into thematically-grouped parameters, we are doing an **ANOVA-like decomposition** of variation in the dependent variable. We introduced this approach in chapter 7 without directly referring to ANOVA and have been using this approach to understand variation in our predictors in most of our models since then.  Here's what Gelman and Hill have to say about the analysis of variance in the context of multilevel models:
...
Batches of parameters that vary a lot from each other reflect large differences in our dependent variable across the parameters. These will be represented by large standard deviations. In contrast, parameters that do not vary much from each other will be represented by small standard deviations. These parameters do not have a large effect on our data (since they do not vary much). 
...
Gelman and colleagues suggest the following general process, that can be referred to as a **Bayesian analysis of variance**, or **BANOVA**:
  ...
  2) Calculate the finite-population (and/or superpopulation) standard deviations for predictors or groups of predictors. 
...
We could potentially add a fifth step: 5) Refit reduced model if some components show little to no meaningful variation, and if you have some compelling reason to do so. This step is not strictly necessary, and may even be a bad idea sometimes, but we will consider it as a possibility. 
...
```{r}
bmmb::getsds (model_height_vtl_f0)
```
...
Getting the finite-sample standard deviations is a bit trickier. First, we should note that although they are called standard deviations, they are actually calculated by finding the root-mean squared error. 

The reason for this is that since we used sum coding,  we assume a mean of 0 for each batch of parameters and our parameters already reflect deviations from the mean. As a result, we do not need to subtract the sample mean from our observations when calculating the standard deviation and instead simply calculate the **root-mean-squared** (RMS) deviation for each batch of coefficients. The root-mean squared is aptly named: It is simply the (square) root of the mean square. In other words, as seen in the formula, it is the square root, of the average of a bunch of squared values. The formula for the RMS is given in \@ref(eq:11-9).

$$
\begin{equation}
RMS = \sqrt {\sum_{i=1}^{n} x_{[i]}^2 / N}
(\#eq:11-9)
\end{equation}
$$

To calculate the standard deviations of different batches of parameters you need to calculate the root-mean squared deviation for each batch, *for each sample*. This means you end up with an estimate of the finite-sample standard deviation for each set of posterior samples. For example, equation \@ref(eq:11-10) has been updated to show the calculation of the RMS of the listener-dependent intercept terms ($L$) across $J$ listeners. The little $^s$ superscript indicates that each calculation is made for an individual posterior sample. As a result, the measure below will result in, for example, 4000 estimates of the RMS for $L$ given 4000 posterior samples. 

$$
\begin{equation}
RMS^s_L = \sqrt {\sum_{j=1}^{J} (L^s_{[j]})^2 / N}
(\#eq:11-10)
\end{equation}
$$


The code below shows how to calculate the finite-population standard deviations for item based on the random effects parameter estimates. 
...
In the above examples, we treated each fixed effect predictor, and the interaction of each of these with listener, as independent. This is actually appropriate for this model because all our predictors *are* conceptually distinct so that we don't really have batches of fixed effects. However, the process Gelman proposes is potentially more complicated that what we've done above. For example, imagine if we had included `C` as a predictor in our model and calculated the listener-dependent effects of `C`. This would result in the fixed effects `C1`, `C2`, and `C3`, 15 `C1:L` random effects, 15 `C2:L` random effects, and 15 `C3:L` random effects. The process described would treat each of the 3 fixed effects as independent, and also treat each batch of 15 random effects as independent. In contrast, the process described by Gelman would treat the 3 category-related fixed effects as a single batch, and the 45 category-related random effects as a single batch. 

The main reason to do it the way way we've shown above is because it can be done easily for all models, and you still get very useful information from the analysis. If you do want to investigate the variation associated with entire clusters of multiple predictors at a time, please see the Gelman articles linked to above. Although the complete method is a straightforward extension of what we've discussed here, there are a few important additional details (e.g., the need to 'recover' missing parameters, the need to calculate 'degrees of freedom', etc.). 

### Using the `banova` function

The `bmmb` package contains a function called `banova` that can get the finite-sample or superpopulation standard deviations for you. This function will only calculate standard deviations the 'simple' way outlined in the previous section, treating each fixed effect and each bundle of random effects as an independent batch. The output of the `banova` function is a single data frame that contains a summary of the standard deviations for fixed and random effects, and the error term if the model contains one. Below we use the function to get both kinds of standard deviation, compared in the previous section.
...
The output of the `banova` function can be used to make a Bayesian ANOVA plot of our model using the `banovaplot` function in the `bmmb` package. An example of a 'BANOVA' plot is shown in figure \@ref(fig:F11-7). These plots compare the (absolute) values of your fixed effects, the RMS deviation of different batches of random effects, and you residual error ($\sigma$), for models that contain an error term. This is effectively the information shown in figure \@ref(fig:F11-5) save for the inclusion of the 'fixed' effects in the figure. If we were to make a BANOVA plot right after fitting our model, we would have a pretty good idea of which of our predictors are associated with meaningful variation in our dependent variable.
...
z <- outer(x, y, f1)
persp(x, y, ztop(z), col = bmmb::cols[13],theta = 0,zlim = c(0,1))
persp(x, y, ztop(z), col = bmmb::cols[13],theta = -90,zlim = c(0,1))
persp(x, y, ztop(z), col = bmmb::cols[13],theta = 45,zlim = c(-0,1))

```

When our models include interactions between quantitative predictors (i.e. the cross-product), our predicted logits will no longer vary along planes. Instead, the surface will resemble a saddle shape based on the sign of the parameter and its magnitude relative to the slopes of the relevant quantitative predictors in the model (top row, figure \@ref(fig:F11-10)). When we convert these saddle shapes to probabilities using the antilogit function, the resulting shape can be strange. For example, in the bottom row in figure \@ref(fig:F11-10), we see that saddle shapes can results in non-contiguous sections of the  associated with 'successes', with a 'canyon' in the middle associate with failures.
...
As for our height model, we'll leave more detailed research questions for chapter 13. For now we are only interested in two questions based on the skills we've been learning: 

Q1) Can we use what we've learned so far to fit and evaluate a logistic regression model with two quantitative predictors?

Q1) Can we extend what we've learned so far to use our logistic model to classify speakers along two stimulus dimensions (f0 and VTL)?
...
We can inspect the model fixed effects using `brmplot` in figure \@ref(fig:F11-11). Beside that we make a BANOVA plot comparing the model fixed effects and the finite-sample standard deviations for 'batches' our random effects parameters.
...
We can use the above method to calculate the category boundaries implied by a model including a cross product. The intersection between a saddle shape and the plane where $z=0$ will not be a straight line. Instead, it will be a **hyperbola** a shape that resembles a pair of parabolas that approach, but never cross an asymptote. Since saddle shapes can fall and then rise again, a surface of this kind may intersect with the plane at $z=0$ in more than one location (seen in figure \@ref(fig:F11-10)). To find the intersection of the plane at $z=0$ and a saddle shape we again set $z=0$, and solve for $y$ as shown in \@ref(eq:11-13).
...
The $a, b, c$ and $d$ parameters above represent model coefficient for the intercept, VTL slope, f0 slope, and cross-product respectively. Below we get the samples for our fixed effects parameters from our model and collect the marginal or 'main effect' estimates of these parameters. These are the *overall* values of the intercept ($a$), the VTL slope ($b$), the f0 slope ($c$), and the cross product of VTL and f0 ($d$),
...
We can also add up appropriate combinations of these fixed effects to calculate separate $a, b, c$ and $d$ parameters for apparent adults and for apparent children. We show and example of this below. Note that in each case we combine the necessary parameters first and then find the average across the samples of the parameter. 
...
In the hopes of finding a model that 'makes more sense', we're going to drop he cross-product from our model. Below we fit a reduced model that is otherwise the same as our initial model, but without the interaction between f0 and VTL. 
...
We find the $a,b,c$ parameters for response plane given our model, calculating the overall (marginal) plane, a plane for apparent children, and another for apparent adults. There is no $d$ parameter this time since there is no cross-product term in this model. 
...
In contrast to the reduced model, we see that the linear boundaries do not differ much in slope in our full model (left). This is likely because either the cross-product, or the interaction of this with apparent age, is able to capture whatever difference our reduced model represents with differences in the effect of f0.  
...
The comparison suggests the model that includes the interaction does a better job of explaining our data, but the differences between the two models is not terribly large (just under two standard errors).

```{r, eval = TRUE}
brms::loo_compare (model_gender_vtl_f0, model_gender_vtl_f0_reduced)
```

However, when adding the `loo` criterion to our models we got error messages like this:
...
When we add the `loo` criterion to our model, `brms` (and `stan`) don't actually fit a new model for every left out data point (i.e. a 'real' leave-one-out cross validation). Instead, as discussed in section X , there is a way to approximate what the model would look like had it *actually* been refit without each data point. This approximation is does using something called **Pareto smoothed importance sampling** (**PSIS**). The Pareto-$\hat{k}$ (`pareto_k`) statistic is a diagnostic for this method that helps you check the assumptions underlying the leave-one-out approximation (cite, Vehtari, Gelman and Gabry, 2017). 
...
Before continuing we will quickly review some information previously discussed (in more detail) in section X (loo elpd parts). The number of estimated parameters in a model is the total number of parameters estimated across all fixed effects, random effects, and 'distributional' parameters such as $\sigma$ or $\nu$. We may distinguish this from the *effective* number of parameters which takes the actual flexibility of the model into account, `p_loo` when calculated using `loo`. If a model shows little variability in its random effects, its effective number of parameters may be smaller than its total number of estimated parameters. Aki Vehtari (cite webpage) outlines three general situations that cause Pareto $\hat{k}$ statistics to be large:
...
In each case, a posterior predictive check may help understand the issues, as can inspecting the distribution of Pareto $\hat{k}$ statistics. Below, we print the actual and effective number of parameters for our full model, as well as our number of observations.
...
curve ((-b_adult*x - a_adult) / (d_adult*x+c_adult), from = -6, xlab="Vocal-tract length (cm)",
       ylab = "f0 (Hz)", to = -c_adult/d_adult-.01, add = FALSE, lwd=2, lty=1,col=1, 
       xlim = c(-2.2,3), ylim = c(-1.2,1.1))
