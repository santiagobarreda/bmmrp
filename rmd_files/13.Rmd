...
We will begin with the perception of apparent gender in adults. [@@ SB - discuss sex v gender. ]The most important predictors of apparent gender in adults are f0 and VTL (cite). Because adult men and women are well separated along these dimensions, listeners can identify the gender of adult speakers from voice in a large majority of cases (e.g. >95% in blank). One important thing to keep in mind that the perception of gender in voice is more complicated than what can be explained by f0 and VTL alone and involves performative aspects that cannot be explained solely due to average differences in body types (cite). 
...
```{r}
brms::pp_check(height_model, ndraws = 50)
```
...
p_check (height_model)
p_check (height_model_priors)



y_hat = predict_one (height_model)

boxplot (exp_data$height ~ exp_data$C_v)
boxplot (y_hat ~ exp_data$C_v)


py = predict (height_model, ndraws = 10)
