...
We're going to present an example of a research paper using models similar to those fit in chapter 11. The models we are going to discuss below present independent analyses of their respective dependent variables, despite the fact that these were provided at the same time and are obviously related. Our intention is simply to provide examples of analyses of both a quantitative variable and a dichotomous variable. Since these analyses involve similar research questions and the same methods, it didn't make sense to repeat the introductions and methods twice. However, we will present independent results and discussion sections for the two experiments.  

Our height model will include apparent gender as a predictor and our apparent gender model will include apparent age as a predictor, which we know co varies strongly with apparent height (see chapter 9?). By analyzing these independently we are adopting the position of two hypothetical researchers: One interested in investigating apparent height and the other interested in investigating apparent gender. We do this as a pedagogical device to give two complete examples of typical analyses in many fields. 

If we think about our two hypothetical researchers above, one of these researchers treats gender as given in order to understand height, and the other treats height as given in order to understand gender. We might consider a third researcher who doesn't want to take either for granted, and wants to understand the way apparent height and gender affect *each other*, including getting to the bottom of which variable affects which. This sort of thing is possible with `stan` and `brms`, however, this being the last chapter and all, it's not something we'll be getting to in this book. Luckily, building these models is a modest extension of the things we cover learned in this book. For more information on models like this, please see (cite). 
...
rect (9.5,140,12.5,160,lwd=2,border="forestgreen",lty=1)
rect (17.5,155,20.5,185,lwd=2,border="forestgreen",lty=1)
...
points (height_data$height[c(29,10,38,19)],  tapply (exp_data$f0_original, exp_data$C, mean), pch=1,lwd=4, 
        cex = 3,col=bmmb::cols[2:5])

```

### Variation in vocal tract resonance between speakers
...
Acoustic resonance occurs when a sound wave bounces back and forth in an enclosed cavity so the energy can't leave the system easily, and builds up inside. Resonance forms the basis of most musical instruments and animal communication systems. So, dogs, cats, and birds, but also guitars and pianos, all make sounds by containing structures that result in acoustic resonances. Most of these things make sounds by putting energy into a tube or enclosed space, and taking advantage of the resonance properties of that enclosed structure. For example, an acoustic guitar uses its strings to introduce energy into the cavity in its body, which serves to substantially amplify the sound. Without this amplification, the guitar strings alone would only make a very quiet and unappealing sound. 
...
Let's imagine the number of times a puff of air can bounce back and forth between the ends of the vocal tract in 1 second. Since the speed of sound is fixed, the repetition rate of this cycle will be entirely determined by the length of the vocal tract. A puff of air will take twice as long to repeat a single cycle along a vocal tract that is twice as long. Because of the reliable connection between the length of a resonator and the frequencies it resonates, we can estimate the length of the vocal tract, in centimeters, based on its resonances. However, it's important to know that this is just an *estimate* based on a *model*, it is not an actual measurement of the physical speaker's vocal tract in units of distance. We could have left this as a unit of frequency scaling, however we have found that readers find VTL in cm more relatable than an equivalent measurement based on the acoustic resonances expressed in frequency units.  

In figure \@ref(fig:F13-2)
...
rect (9.5,140,12.5,160,lwd=2,border="forestgreen")
rect (17.5,155,20.5,185,lwd=2,border="forestgreen")
...
Our discussion of the perception of apparent speaker characteristics will focus on three of these: Apparent height, apparent age, and apparent gender. We will begin with the perception of apparent gender in adults. [@@ SB - discuss sex v gender. ]The most important predictors of apparent gender in adults are f0 and VTL (cite). Because adult men and women are well separated along these dimensions, listeners can identify the gender of adult speakers from voice in a large majority of cases (e.g. >95% in blank). One important thing to keep in mind that the perception of gender in voice is more complicated than what can be explained by f0 and VTL alone and involves performative aspects that cannot be explained solely due to average differences in body types (cite). 
...
Each of these response variables were analyzed using multilevel Bayesian models fit with `stan` (cite), using the `brms` (cite) package in R (cite). 
...
listener_gender_effects = 
  bmmb::short_hypothesis(height_model, "G1=0",scope="coef",group="L")

brmplot (listener_gender_effects[,1:4]*2)
```

Finally, before concluding our discussion of apparent height judgments we want to discuss the ...
...
Figure below presents a boxplot showing all size responses across all listeners for speakers judged to be girls, women, men, and boys respectively. There are clear systematic differences in apparent height across the different apparent speaker categories. As seen in the middle and right plots of the same figure, it appears that these between-category differences may be associated with differing voice characteristics across speakers in different categories. 
...
```{r F13-5, fig.height = 3.5, fig.width = 8, fig.cap='A comparison of fixed effect estimates provided by the brms (red) and lmer (black) models. The brms intervals are the 95% credible intervals, those for lmer are twice the standard error of the parameter estimate.', echo = FALSE, cache = TRUE}
...
brmplot (ranef(gender_model)$S[,,1], col = mod_cat)

```



## Discussion: Apparent gender


As a reminder, the questions we posed above were: 
...
We will base our answers to these questions on our reduced model above (`model_gender_vtl_f0_reduced`). Based on our discussion above we conclude that listeners *do* use f0 and VTL, but the jury is still out on the interaction of the two. It seems that to really get a handle on how listeners respond to the sorts of voices not included in this experiment (i.e. voices in the bottom left and top right of figure \@ref(fig:F11-13)), we would need to run an experiment with more voices in these areas of the stimulus space. Below we print a table of the fixed effects for our reduced model:

```{r T11-1}
knitr::kable(fixef(model_gender_vtl_f0_reduced), caption = "Fixed effect posterior means, error, and credible intervals for our reduced model.",digits = 2)
```

It seems that differences in apparent speaker age is associated with varying use of both VTL and f0. In each case, the effects of f0 and VTL changes are *stronger* for adults compared to children. For example, the interaction between f0 and apparent age (`f0:A1`) has the effect of reducing the effect of f0 to nearly zero for children ($f0 + (-A1) = 2.2 - 1.6 = 0.6$) and nearly doubling it for adults ($f0 + A1 = 2.2 + 1.6 = 3.8$). We noted in chapter 10 (section \@ref(c10-classification)) that slopes of larger magnitudes generally indicate steeper category boundaries and less uncertainty between categories along that dimension. In this case, the larger slope magnitudes for apparent adults likely reflect the fact that men and women are more discriminable based on their fo and VTL than boys and girls. 
