...
This book starts with multilevel models and doesn't looks back. It focuses on a realistic and complicated data set, and focuses from (nearly) the start (chapter 4) on realistic models that could actually be used in publishable research today.
...
The `brms` package is a useful way for us to use the *Stan* programming language, the real star of the show. We could write our own models for all the of analyses presented in this book and fit them directly in *Stan*. However, `brm` will do this for us quickly, easily, and without making mistakes. It also writes highly efficient models that fit quickly in *Stan*. In addition, the helper functions in the package make working with the posterior samples very simple, and work well with a wide range of other packages related to *Stan*. However, despite the fact that this book centers entirely about the use of `brms`, we don't really think of this book as being *about* `brms`. The information presented in this book applies to modeling more generally, and is also useful for people that write their own models directly in *Stan*. The main difference is these readers would need to do a lot of things 'by hand' or find other solutions to many of the things the `brms` package makes very simple to do. 
